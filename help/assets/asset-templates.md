...
Les modèles de ressources offrent une flexibilité permettant d’apporter des modifications limitées tout en garantissant la sécurité des éléments graphiques globaux. Cette capacité en fait des composants idéaux pour adapter et diffuser rapidement du contenu dans différents services. La réutilisation de contenu permet de réduire les coûts de gestion des canaux papier et numériques. Cela garantit en outre une expérience globale cohérente, quel que soit le canal de diffusion.
...
* Modifier les champs modifiables d’un modèle depuis l’interface web.
...
Vous pouvez modifier un dérivé immédiatement après sa création. Vous pouvez également l’ouvrir depuis la page [!UICONTROL Modèles] ou la page de la ressource.
...
1. Pour ouvrir un dérivé pour le modifier, procédez de l’une des façons suivantes :
   ...
   * Sélectionnez le dérivé et cliquez sur **[!UICONTROL Modifier]** dans la barre d’outils.
   ...
   >[!NOTE]
   >
   >Intégrez votre déploiement [!DNL Experience Manager] avec un [!DNL InDesign Server] pour qu’[!DNL Experience Manager Assets] puisse extraire les données du modèle [!DNL InDesign] et les rendre modifiables. Pour plus d’informations, consultez [Intégration d’Experience Manager Assets à InDesign Server](/help/assets/indesign.md).
