...
1. Ajouter `/aem65/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` chemin d’accès au fichier conf à l’aide de `cat SWitchEngineLibs.conf` .

1. Exécuter `ldconfig` pour créer les liens et le cache nécessaires.

1. Dans le compte utilisé pour démarrer [!DNL Experience Manager], modifier `.bash_profile` fichier . Ajouter `LD_LIBRARY_PATH` en ajoutant ce qui suit.
...
1. Dans [!DNL Experience Manager] interface utilisateur, sélectionnez **[!UICONTROL Outils]** > **[!UICONTROL Workflow]** > **[!UICONTROL Modèles]**.
