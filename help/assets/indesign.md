...
* un proxy pour distribuer la charge de certaines tâches de traitement. Un proxy est une instance [!DNL Experience Manager] qui communique avec un proxy worker pour exécuter une tâche spécifique, ainsi qu’avec d’autres instances [!DNL Experience Manager] pour transmettre les résultats.
...
1. Un cadre d’exécution envoie des scripts de commande au [!DNL InDesign Server] via le protocole SOAP (Simple Object Access Protocol).
Ce script de commande permet d’effectuer les opérations suivantes :
   ...
   * Republier les fichiers résultants dans [!DNL Experience Manager Assets].
   ...
   >[!NOTE]
   >
   >IDML est un format XML qui effectue le rendu de tout le contenu du fichier [!DNL InDesign]. Ils sont stockés sous forme de paquet compressé à l’aide de la compression [ZIP](https://techterms.com/definition/zip). Pour plus d’informations, consultez les [Formats d’échange d’InDesigns INX et IDML](https://www.peachpit.com/promotions/adobe-creative-cloud-2024-release-books-ebooks-and-142536).

   >[!CAUTION]
   >
   >Si [!DNL InDesign Server] n’est pas installé ou configuré, vous pouvez tout de même charger un fichier INDD dans [!DNL Experience Manager]. Cependant, les versions générées se limitent aux formats PNG et JPEG. Il n’est pas possible de générer de rendus HTML, `.idml` ou de pages.
...
1. À partir de la ligne de commande, démarrez le serveur :
   ...
   Cela démarre le serveur avec le plugin SOAP à l’écoute sur le port 8080. Tous les messages de journal et les résultats sont écrits directement dans la fenêtre de commande.
...
Pour personnaliser, modifiez l’onglet **[!UICONTROL Arguments]** de l’étape **[!UICONTROL Extraction de médias]**.
...
>[!CAUTION]
>
>Ne modifiez pas la bibliothèque ExtendScript. Cette bibliothèque fournit la fonctionnalité HTTP requise pour communiquer avec Sling. Ce paramètre spécifie la bibliothèque à envoyer vers le [!DNL InDesign Server] pour y être utilisée.
...
* **Chemin racine de la page** : chemin d’accès à la racine de la page résultante. Si ce champ est laissé vide, le nœud contenant les rendus de la ressource est utilisé.
...
* Lorsque IDS est exécuté sur plusieurs machines, il faut compter le nombre total de processeurs disponibles (c’est-à-dire sur l’ensemble des machines), puis soustraire le nombre total de machines.
