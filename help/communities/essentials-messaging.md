...
description: Découvrez les détails de l’utilisation et de l’utilisation du composant Messagerie pour inclure une fonctionnalité de messagerie sur un site web.
...
>[!CAUTION]
>
>Le paramètre de chaîne ne doit *pas* contenir de barre oblique « / » pour les méthodes MessageBuilder suivantes :
>
>* `setInboxPath`()
>* `setSentItemsPath`()
>
...
Une structure de site de communauté, créée à l’aide de l’assistant, inclut la fonctionnalité de messagerie lorsqu’elle est sélectionnée. Voir `User Management` paramètres de la [console Sites de la communauté](/help/communities/sites-console.md#user-management).

### Exemple De Code : Notification De Réception De Message {#sample-code-message-received-notification}

La fonctionnalité de messagerie sociale déclenche des événements pour les opérations, par exemple `send`, `marking read`, `marking delete`. Ces événements peuvent être capturés et des actions peuvent être entreprises sur les données contenues dans l&#39;événement.

L’exemple suivant illustre un gestionnaire d’événements qui écoute l’événement `message sent` et envoie un e-mail à tous les destinataires du message à l’aide de l’`Day CQ Mail Service` .
...
1. Créez un `bundle node`dans `/apps/engage/install` avec des noms arbitraires, tels que :
   ...
   * Nom : Prise En Main De La Notification Du Message Du Tutoriel
   * Description : exemple de service permettant d&#39;envoyer une notification par e-mail aux utilisateurs lorsqu&#39;ils reçoivent un message
...
1. Accédez à `/apps/engage/install/com.engage.media.social.messaging.MessagingNotification/src/main/java/com/engage/media/social/messaging/notification`, puis :
   ...
   1. Créez des `MessageEventHandler.java` de classe.
   1. Copiez et collez le code ci-dessous dans `MessageEventHandler.java`.

1. Cliquez sur **Enregistrer tout**.
1. Accédez à `/apps/engage/install/com.engage.media.social.messaging.MessagingNotification/com.engage.media.social.messaging.MessagingNotification.bnd` et ajoutez toutes les instructions d’importation telles qu’écrites dans le code `MessageEventHandler.java`.
1. Créez le lot.
1. Assurez-vous que le service `Day CQ Mail Service`OSGi est configuré.
1. Connectez-vous en tant qu’utilisateur de démonstration et envoyez un e-mail à un autre utilisateur.
1. Le destinataire reçoit un e-mail concernant un nouveau message.
