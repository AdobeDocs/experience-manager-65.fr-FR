---
title: Scores et badges des communautés
seo-title: Scores et badges des communautés
...
Le service de notation décerne aux membres de la collectivité des badges de récompense en fonction des règles appliquées à leur   de la collectivité.

Pour que les badges apparaissent comme une récompense pour  , deux choses doivent se produire :
...
Lorsqu’ils sont installés à partir de la console Badges, les badges personnalisés sont automatiquement répliqués dans le  de publication .
...
* Identifiez les règles relatives aux points de gain (règles[de](#scoring-rules)notation).
...
Pour activer le score et les badges, ajoutez les propriétés `scoringRules` et `badgingRules` à n’importe quel noeud de l’arborescence de contenu du site.
...
|---|---|---|
| badgingRules | Chaîne[] | un de tableau de règles de [marquage](#badging-rules) |
| scoringRules | Chaîne[] | un tableau  de règles de [notation](#scoring-rules) |
...
Les règles de notation et de bourrage ne sont en vigueur que pour les instances de composants qui ont activé le badge en modifiant la configuration du composant en mode [de](/help/communities/author-communities.md)création.
...
Très simplement, chaque règle de score est un  d’une ou de plusieurs sous-règles. Des règles de score sont appliquées au contenu du site de la communauté afin d’identifier les règles à appliquer lorsque les badges sont activés.

Les règles de score sont héritées mais pas additifs. Par exemple :

* Si page2 contient la règle de score2 et que sa page1 ancêtre contient la règle de score1.
...
Les règles de score sont des noeuds de type `cq:Page` avec des propriétés sur son `jcr:content` noeud qui spécifient le  des sous-règles qui le définissent.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ul>
     <li>requis; le verbe correspond à une action </li>
     <li>il doit y avoir au moins une propriété verb</li>
     <li>le verbe doit être saisi en MAJUSCULE</li>
     <li>il peut exister plusieurs propriétés de verbe, mais aucun </li>
     <li>la valeur est le score à appliquer pour ce </li>
     <li>la valeur peut être positive ou négative</li>
     <li>un  de verbes pris en charge dans la version se trouve dans la section <a href="#topics-and-verbs">Rubriques et verbes</a> .</li>
    </ul> </td>
  </tr>
  <tr>
   ...
   <td>
    <ul>
     <li>facultatif; restreint les sous-règles aux composants de la communauté identifiés par des sujets </li>
     <li>if specified : est une chaîne de plusieurs valeurs de  de rubriques</li>
     <li>un de rubriques de la version se trouve dans la section <a href="#topics-and-verbs">Rubriques et verbes</a> .</li>
...
Toute modification ou tout ajout apporté aux règles de notation ou aux sous-règles effectuées dans le de l’auteur  doit être installé lors de la publication.
...
Les règles de badge consistent en une `thresholds` propriété obligatoire qui est un ordonné de scores mappés sur des badges. Les scores doivent être triés en valeur croissante. Par exemple :
...
* `60|/etc/community/badging/images/silver-badge/jcr:content/silver.png`
   ...
   * Un badge d&#39;argent est décerné lorsque 60 points ont été accumulés.
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><em>(obligatoire)</em> Chaîne à plusieurs valeurs du formulaire "number|path"
    ...
    </ul> Les chaînes doivent être triées afin que les nombres augmentent en valeur et qu’aucun espace blanc ne s’affiche entre le nombre et le chemin.<br /> Exemple d’entrée :<br /> <code>80|/etc/community/badging/images/gold-badge/jcr:content/gold.png</code></td>
  </tr>
  <tr>
   <td>badgingType</td>
   <td>Chaîne</td>
  ...
  <tr>
   <td>scoringRules</td>
   <td>Chaîne[]</td>
   <td>(<em>facultatif</em>) Chaîne à plusieurs valeurs pour limiter la règle de badge au de notation identifié par les règles de notation</td>
  </tr>
 </tbody>
</table>
...
Toute modification ou tout ajout apporté aux règles de mise en badge ou aux images effectuées dans le de l’auteur   doit être installé lors de la publication.
...
cURL -i -X POST -H *header* -u *signature* -F *opération* -F *badge membre--url***
...
*badge-image-file* = emplacement du fichier image de badge dans le référentiel, par exemple : /etc/community/badging/images/modérator/jcr:content/moderator.png

*members--url* = point de terminaison du du membre sur la publication, par exemple : https://&lt;serveur>:&lt;port>/home/users/community/riley/profile.social.json
...
>* Il peut s’agir d’un nom aléatoire et obscur - voir Liste de contrôle [de](/help/sites-administering/security-checklist.md#verify-that-you-are-not-disclosing-personally-identifiable-information-in-the-users-home-path) sécurité concernant l’ID autorisé.
>
...
#### Affectation d’un badge de modérateur {#assign-a-moderator-badge}
...
Il est possible de créer des règles de notation et de mise en badge pour les composants personnalisés en associant les rubriques  du composant créées pour le composant aux verbes.

## Rubriques et verbes {#topics-and-verbs}

Lorsque les membres interagissent avec les fonctionnalités des communautés, des  de sont envoyées, qui peuvent déclencher des écouteurs asynchrones, tels que des notifications et des scores.

L’instance SocialEvent d’un composant enregistre le  du comme `actions` cela se produit pour une `topic`. SocialEvent inclut une méthode pour renvoyer une `verb` associée à l’action. Il existe une relation *n-1* entre `actions` et `verbs`.

Pour les composants de communautés distribués, les tableaux suivants décrivent la `verbs` définition pour chaque `topic` élément disponible pour l’utilisation dans les sous-règles [de](#scoring-sub-rules)notation.
...
**[Composant](/help/communities/blog-feature.md)**de SocialEvent`topic`= com/adobe/cq/social/
...
###  de composants personnalisés {#custom-component-events}

Dans le cas d’un composant personnalisé, un événement SocialEvent est appelé pour enregistrer le  du composant comme `actions` cela se produit pour un `topic`.

Pour prendre en charge la notation, SocialEvent doit remplacer la méthode `getVerb()` afin qu’une valeur appropriée `verb` soit renvoyée pour chaque `action`. Le `verb` retour d’une action peut être un élément couramment utilisé ( `POST`) ou spécialisé pour le composant ( `ADD RATING`). Il existe une relation *n-1* entre `actions` et `verbs`.

## Résolution des incidents {#troubleshooting}

### Les badges ne s’affichent pas {#badges-are-not-appearing}

Si des règles de notation et de badge ont été appliquées au contenu du site Web, mais que les badges ne sont attribués à aucun , assurez-vous que les badges ont été activés pour l’instance de ce composant.
...
* Accédez à CRXDE Lite sur l’auteur.
* Accédez à la page de base :

   * /content/sites/interaction/fr/jcr:content

* Ajouter la propriété badgingRules :
...
* Ajouter la propriété scoringRules :
...
* A nouveau l’utilisation de CRXDE Lite.
* Accédez au composant de forum

   * `/content/sites/engage/en/forum/jcr:content/content/primary/forum`

* Ajouter la propriété booléenne allowBadges, si nécessaire, et assurez-vous qu’elle est vraie.
