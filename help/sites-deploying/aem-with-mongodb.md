...
* `customBlobStore`
Si le déploiement stocke les fichiers binaires dans la base de données, ils feront partie du jeu de travail. Pour cette raison, il est conseillé de ne pas stocker les fichiers binaires dans MongoDB, en choisissant un magasin de données secondaire tel qu’un 
magasin de données `FileSystem` sur un NAS.

* `cache`
Taille du cache en Mo. Elle est répartie entre les différents caches utilisés dans 
