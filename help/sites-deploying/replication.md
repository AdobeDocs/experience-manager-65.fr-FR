...
* **ID utilisateur de l’agent**
  ...
  >[!CAUTION]
  >
  >Pour un agent dans l’environnement de création, ce compte *doit* doit avoir un accès en lecture à tous les chemins que vous souhaitez répliquer.
...
* **Domaine NTLM**
  ...
  Domaine pour l’authentification NTLM.
...
Comme l’environnement de publication se trouve généralement dans la DMZ, pour renvoyer le contenu à l’environnement de création, la connexion doit être établie à partir de l’instance de création. Pour cela, il faut :
...
Si vous rencontrez des problèmes, vous pouvez vérifier les journaux sur l’instance de création. Selon le niveau de détail requis, vous pouvez également définir le **Niveau de journal** sur `Debug` à l’aide de la boîte de dialogue **Paramètres d’agent**, comme indiqué ci-dessus.
...
>1. Configurez un compte utilisateur avec les droits d’accès requis pour lire le contenu répliqué dans cet environnement de publication spécifique.
...
La réplication par lots ne réplique pas individuellement les pages ou les ressources. Il attend plutôt que le premier seuil des deux (basé sur le temps ou la taille) soit déclenché.
...
L’éditeur décompresse tous les éléments, les enregistre et les renvoie à l’auteur.
