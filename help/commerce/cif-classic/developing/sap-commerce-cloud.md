...
>[!NOTE]
>
>Le framework d’e-commerce peut être utilisé avec n’importe quelle solution d’e-commerce. Certaines fonctionnalités et certains exemples traités ici font référence à la solution [hybris](https://www.sap.com/products/crm.html).
...
Le framework d’e-commerce peut être utilisé avec n’importe quelle solution d’e-commerce. Le moteur utilisé doit être identifiable par AEM :
...
| `cq:commerceProvider = geometrixx` | dans une installation AEM standard, une implémentation spécifique est requise. Par exemple, l’exemple Geometrixx, qui inclut des extensions minimales de l’API générique |
|--- |--- |
...
* Lors de l’appel de maven, ajoutez l’argument de ligne de commande suivant à la commande
  ...
  Il télécharge la distribution Hybris 4 préconfigurée et l’incorpore au bundle `cq-commerce-hybris-server`.
...
* L’activation d’une page dans AEM (c) nécessite une vérification (b) et n’est possible que si
  ...
  * le produit se trouve dans une version de catalogue **en ligne** pour les pages produits ;
...
* Les pages produits activées doivent accéder à la version **en ligne** des données de produit (d).
...
Chaque produit et/ou variante est représenté par une ressource et mappe donc 1:1 à un nœud de référentiel. Il s’agit d’un corollaire selon lequel un produit et/ou une variante spécifique peut être identifié de manière unique par son chemin d’accès.
