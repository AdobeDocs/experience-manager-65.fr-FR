...
Cette section répertorie les fonctionnalités qui ont été marquées comme obsolètes dans AEM 6.5. En règle générale, les fonctionnalités dont la suppression est prévue dans une version ultérieure sont d’abord définies comme obsolètes, et une alternative est fournie.
...
Cette section répertorie les fonctionnalités qui ont été supprimées d’AEM 6.5. Ces fonctionnalités étaient signalées comme obsolètes dans les versions antérieures.
...
| Cache | `system/console/slingjsp` a été supprimé et n’est plus disponible dans AEM 6.5. | Les classes et le cache Slightly sont stockés sous le bundle Apache Sling Commons FileSystem ClassLoader. Vous pouvez vérifier le numéro de bundle dans la console web d’AEM et supprimer le dossier de cache directement du système de fichiers (`crx-quickstart/launchpad/felix/bundle<ID>`). |  |
| Screens | Suppression de la prise en charge du bundle activemq et de ses configurations associées. |  |  |
