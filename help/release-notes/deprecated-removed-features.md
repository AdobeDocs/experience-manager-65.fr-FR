...
| Adobe Target | En ajoutant la possibilité pour AEM de se connecter au service Adobe Target à l’aide de la fonction [!DNL Adobe I/O] Basée sur l’API Adobe Target Standard (API REST) dans AEM 6.5, la méthode d’API Target Classic (XML) est obsolète. | Reconfigurez l’intégration pour [utiliser la nouvelle API ;](https://helpx.adobe.com/fr/experience-manager/kt/sites/using/aem-sites-target-standard-technical-video-understand.html). |  |
...
| Composants (AEM Sites) | Adobe ne prévoit pas d’apporter d’autres améliorations à la plupart des composants de base (Foundation) stockés dans `/libs/foundation/components`. Recherchez le `cq:deprecated` et `cq:deprecatedReason` dans le dossier component. Les composants de base sont inclus dans AEM version 6.5. Les clients effectuant une mise à niveau à partir de versions antérieures peuvent continuer à les utiliser en l’état. De plus, les composants de base sont entièrement pris en charge, même s’ils sont obsolètes. | Adobe recommande d’utiliser les composants principaux pour les projets futurs. Les sites existants peuvent rester tels quels ou utiliser la variable [AEM Moderniser la suite d’outils](https://github.com/adobe/aem-modernize-tools) pour refactoriser le site afin d’utiliser les composants principaux. |  |
