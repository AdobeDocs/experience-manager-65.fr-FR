...
Pour communiquer la suppression ou le remplacement imminent des fonctionnalités AEM, les règles suivantes s’appliquent :
...
| Commerce | [CIF REST](https://github.com/adobe/commerce-cif-api) a été fourni en 2018 sous la forme d’un ensemble de microservices permettant l’intégration entre AEM et les moteurs de commerce. Après l&#39;acquisition de Magento par Adobe à la mi-2018, Adobe a décidé de changer d&#39;approche pour deux raisons. Magento possède son propre ensemble d&#39;API de commerce (REST et GraphQL) et il n&#39;est pas recommandé de conserver deux ensembles d&#39;API. Les tendances du marché ont indiqué que les clients se dirigeaient vers GraphQL, parce que c&#39;est un moyen plus efficace d&#39;interroger des données. En 2019, Adobe a publié le nouveau Commerce Integration Framework qui utilise les API GraphQL de Magento comme source de vérité. Adobe n&#39;a pas l&#39;intention d&#39;investir davantage dans CIF REST. Il est vivement conseillé aux clients d’utiliser la solution de remplacement. | Pour les intégrations AEM-Magento, passez à l’archétype [CIF](https://github.com/adobe/aem-cif-project-archetype) AEM et aux composants [principaux](https://github.com/adobe/aem-core-cif-components)AEM CIF. See AEM and Magento integration [using Commerce Integration Framework](https://www.adobe.io/apis/experiencecloud/commerce-integration-framework/integrations.html#!AdobeDocs/commerce-cif-documentation/master/integrations/02-AEM-Magento.md). Le soutien aux intégrations tierces (autres que Magento) avec la nouvelle approche est sur notre feuille de route. |
| Composants (AEM Sites) | Adobe ne prévoit pas d’apporter d’autres améliorations à la plupart des composants de base (Foundation) stockés dans `/libs/foundation/components`. Look for the `cq:deprecated` and `cq:deprecatedReason` property in the component folder. AEM 6.5 inclut les composants Foundation et les clients qui effectuent la mise à niveau depuis des versions antérieures peuvent continuer à les utiliser en l’état. En outre, les Composantes de base sont entièrement prises en charge, même si elles sont obsolètes. | Adobe recommande d&#39;utiliser les composants principaux pour les projets futurs. Existing sites can remain as is or use the [AEM Modernize Tools Suite](https://github.com/adobe/aem-modernize-tools) to refactor the site to use Core Components. |
| Composants (AEM Sites) | Design Importer Components `/libs/wcm/designimporter/components` have been marked as deprecated starting 6.5. Adobe does not plan to make further enhancements to that implementation of the design importer. | Adobe prévoit de proposer une autre mise en oeuvre du cas d’utilisation dans les versions futures. |
| Foundation | Structure de déchargement Granite. Adobe n’envisage pas d’apporter d’autres améliorations au cadre de déchargement introduit dans CQ 5.6.1 pour externaliser le traitement des actifs. | Adobe travaille sur une structure de déchargement native pour le cloud de nouvelle génération. |
...
| Formulaires | L&#39;intégration des AEM Forms avec AEM Mobile est obsolète. | Aucun remplacement n’est disponible. |

## Fonctionnalités supprimées {#removed-features}

Cette section liste les fonctionnalités et fonctionnalités qui ont été supprimées d’AEM 6.5. Les versions précédentes affichaient ces fonctionnalités comme obsolètes.
...
| Formulaires | La mise à niveau par un seul saut de LiveCycle ES4 SP1 vers AEM Forms 6.5 sur JEE n’est pas disponible | Reportez-vous aux chemins [de mise à niveau](../forms/using/upgrade.md) disponibles dans la documentation de mise à niveau AEM Forms. |
| Formulaires | Suppression de la prise en charge de la mise en grappe basée sur UPD de AEM Forms on JEE | Vous pouvez uniquement utiliser la mise en grappe basée sur TCP dans AEM Forms on JEE. Si vous mettez à niveau un serveur de multidiffusion UDP d’une version précédente vers AEM Forms 5.5 sur JEE, effectuez des configurations manuelles pour passer à la mise en grappe de gemfire basée sur TCP. Pour obtenir des instructions détaillées, voir [Mise à niveau vers AEM Forms 6.5 sur JEE.](../forms/using/upgrade-forms-jee.md) |
