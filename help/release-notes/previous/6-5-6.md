...
Adobe Experience Manager 6.5.6.0 est une mise à jour importante qui comprend de nouvelles fonctionnalités, des améliorations importantes demandées par les clients, ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiées depuis la mise à disposition de la version 6.5 en **avril 2019**. Elle peut être installée sur Adobe Experience Manager 6.5.

Les fonctionnalités et améliorations clés introduites dans Adobe Experience Manager 6.5.6.0 incluent :
...
* Le référentiel intégré (Apache Jackrabbit Oak) a été mis à jour vers la version 1.22.4.

Voici la liste des correctifs fournis dans la version 6.5.6.0 d’[!DNL Experience Manager].
...
* La redirection vers la page [!DNL Experience Manager] avec ancre ne fonctionne pas sur l’instance de création, puisque `PageRedirectServlets` place la chaîne de requête après un fragment d’URL ou une ancre (NPR-34288).
...
* La copie d’exécution du modèle de workflow personnalisé des ressources (créé dans `/var/workflow/models/dam`) est supprimée lors de l’installation du pack de services 5 d’[!DNL Experience Manager] 6.5 ou d’une version précédente sur [!DNL Experience Manager] 6.5 (NPR-34532). Pour récupérer la copie d’exécution, synchronisez la copie de conception du modèle de workflow avec la copie d’exécution à l’aide de l’API HTTP :
...
* Les ressources scintillent lors du chargement de la page lorsque vous passez d’une option à l’autre du rail latéral (par exemple, [!UICONTROL Présentation], [!UICONTROL Chronologie], [!UICONTROL Visionneuses]) dans la page des détails de la ressource (NPR-34235).
...
* Les modifications des paramètres d’image prédéfinis dans [!UICONTROL Adobe Experience Manager] ne sont pas synchronisées avec Scene7 Publishing System (CQ-4299713).
...
Après l’installation du package complémentaire [!DNL Experience Manager Forms] 6.5.6.0 :
...
### Mise en cache améliorée pour les pages de formulaires adaptatifs traduits (6.5.6.0) {#enhanced-caching-translated-adaptive-forms}
...
UberJar pour Experience Manager 6.5.6.0 est disponible dans le [Référentiel Maven central](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.6/).
