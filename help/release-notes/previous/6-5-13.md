---
title: Notes de mise à jour d’ [!DNL Adobe Experience Manager] 6.5.13.0
...
[!DNL Experience Manager] 6.5.13.0 comprend de nouvelles fonctionnalités, des améliorations importantes demandées par les clientes et clients, ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiées depuis la version initiale 6.5 en avril 2019. [Installez ce pack de services](#install) dans [!DNL Experience Manager] 6.5.
...
Les correctifs, fonctionnalités clés et améliorations suivants ont été introduits dans [!DNL Experience Manager] 6.5.13.0 :
...
* Lors de la création d’un paramètre d’image prédéfini pour Dynamic Media dans [!DNL Experience Manager], le nombre maximal autorisé est limité à 2 000 x 2 000 pixels dans l’interface utilisateur. Lorsque la valeur est augmentée à 2 001 pixels pour la largeur ou la hauteur, le bouton **[!UICONTROL Enregistrer]** est désactivé. (ASSETS-5691)
* L’utilisateur peut empêcher le chargement de certains formats de fichier vers Dynamic Media. (ASSETS-5693)
* Accessibilité : les personnes ayant un problème visuel et qui dépendent des lecteurs d’écran sont affectées si l’attribut Alt n’est pas implémenté sur une image dans l’interface utilisateur des paramètres d’image prédéfinis. (ASSETS-9817)
...
* Accessibilité : la sélection du clavier ne passe pas au bouton « Rétablir » qui s’affiche après avoir appelé le bouton « Annuler » sur la page de l’éditeur d’ensemble d’images, lors de la navigation à l’aide de la touche de tabulation du clavier. (ASSETS-5582)
* Accessibilité : les personnes qui dépendent des lecteurs d’écran sont affectées, car l’attribut Alt n’est pas fourni pour une image d’ensemble d’images qui se trouve sous l’en-tête Propriétés. (ASSETS-5576)
...
* Lorsque vous utilisez un workflow de [!DNL Experience Manager] pour envoyer un e-mail en hébreu, le message reçu par l’utilisateur final contient des points d’interrogation (??) au lieu du texte en hébreu (NPR-38296).
...
* [!DNL Experience Manager] 6.5.13.0 nécessite [!DNL Experience Manager] 6.5. Consultez la [documentation de mise à niveau](/help/sites-deploying/upgrade.md) pour obtenir des instructions détaillées.
...
* Lors d’un déploiement avec MongoDB et plusieurs instances, installez [!DNL Experience Manager] 6.5.13.0 sur l’une des instances de création à l’aide du gestionnaire de modules.

>[!NOTE]
>
>Adobe recommande de ne pas supprimer ni désinstaller le package d’[!DNL Experience Manager] 6.5.13.0.
...
1. La page d’informations sur les produits (`/system/console/productinfo`) affiche la chaîne de version de mise à jour `Adobe Experience Manager (6.5.13.0)` sous [!UICONTROL Produits installés].
...
1. Le bundle OSGi `org.apache.jackrabbit.oak-core` est de la version 1.22.11 ou ultérieure (utiliser la console Web : `/system/console/bundles`).
...
* [Liste des modules de contenu inclus dans Experience Manager 6.5.13.0](/help/release-notes/assets/65130_packages.txt)
