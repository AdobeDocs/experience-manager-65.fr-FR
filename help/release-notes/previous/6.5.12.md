...
* Après la configuration d’une connexion entre les déploiements de gestion des ressources numériques et Sites à distance, les ressources de gestion des ressources numériques à distance sont disponibles pour le déploiement sur Sites. Vous pouvez désormais effectuer les opérations suivantes : mettre à jour, supprimer, renommer et déplacer des opérations sur les ressources ou dossiers de gestion des ressources numériques à distance. Les mises à jour sont disponibles avec un certain retard automatiquement sur le déploiement de Sites (NPR -37816).
...
* L’environnement de création présente des problèmes d’interface utilisateur, tels que le titre de la page Mode d’édition qui n’est pas centré, et pour le sélecteur de composants autorisés dans l’éditeur de stratégies, la case à cocher du groupe occupe toute la largeur du conteneur. Le libellé est donc renvoyé à la ligne suivante (NPR-37878).
* [Plateforme] Le numéro de version de xmlns:metatype dans le fichier metatype.xml de commons-httpclient est &quot;https://www.osgi.org/xmlns/metatype/v1.0.0&quot; au lieu de &quot;https://www.osgi.org/xmlns/metatype/v1.2.0&quot; (NPR-37865).
...
* Le multichamps ne s’affiche pas correctement dans le conteneur de mises en page et renvoie une erreur (NPR-37811).
* Toute tentative de redimensionnement de la mise en page du composant dans l’éditeur de page ne fonctionne pas dans la mise en page pour périphériques mobiles (NPR-37805).
...
* Le conteneur de mises en page dans un modèle de page SPA ne peut pas enregistrer les classes CSS personnalisées définies dans la stratégie de modèle lors de l’exécution des pages SPA React (NPR-37697).
...
* Lors de l’utilisation de ressources connectées, un utilisateur non administrateur peut rechercher une ressource même si les utilisateurs non administrateurs sont limités à l’accès au référentiel de gestion des ressources numériques (NPR-37644).
...
* Lors d’un déploiement avec MongoDB et plusieurs instances, installez Experience Manager 6.5.12.0 sur l’une des instances d’auteur à l’aide du gestionnaire de modules.
...
>[!NOTE]
>
>Passez cette étape si vous n’utilisez pas Experience Manager Forms. Les correctifs dans Experience Manager Forms sont fournis par le biais d’un module complémentaire séparé une semaine après date de publication du pack de services [!DNL Experience Manager].
