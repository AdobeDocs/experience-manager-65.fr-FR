...
* Une nouvelle colonne triable pour la date de création a été ajoutée en mode Liste de la gestion des ressources numériques et dans les résultats de recherche de ressources en mode Liste.
...
* Une nouvelle colonne triable pour la date de création a été ajoutée en mode Liste de la gestion des ressources numériques et dans les résultats de recherche de ressources en mode Liste (NPR-31312).
...
* L’élément d’action de Sélection de dossier Créer > Charger un fichier est manquant lorsqu’une liste ACL de refus de `jcr:removeChildNodes` et `jcr:removeNode` sur le chemin d’accès est appliquée à un utilisateur (NPR-30840).
...
* Le RTE ne permet pas d’insérer un tableau incorporé en tant qu’élément de liste (NPR-30879).
...
* Lorsqu’un utilisateur ajoute un champ Transfert de fichier à un champ à plusieurs champs, le chemin d’accès à l’image est stocké dans le nœud de composant au lieu du nœud à plusieurs champs (NPR-30882).
...
* Lors du passage du mode Carte au mode Liste sur une page de résultats de recherche, un décalage s’affiche avant de pouvoir faire défiler la page (NPR-31286).

* La case à cocher [!UICONTROL Tout sélectionner] est masquée en mode Liste dans l’interface utilisateur de [!DNL Sites] (NPR-31614).
...
>[!NOTE]
>
>Le pack de services [!DNL Experience Manager] n’inclut pas de correctifs pour [!DNL Experience Manager Forms]. Les correctifs sont fournis à l’aide d’un module complémentaire Forms distinct. En outre, un programme d’installation cumulatif est publié, qui inclut des correctifs pour [!DNL Experience Manager Forms] sous JEE. Pour plus d’informations, consultez [Installation d’Experience Manager Forms sous JEE](/help/release-notes/jee-patch-installer-65.md).
...
>[!NOTE]
>
>Pour les clients [!DNL Experience Manager Forms], il est impératif d’installer le module complémentaire [!DNL Experience Manager Forms] après l’installation de tout pack de services, pack de correctifs cumulés ou pack de fonctionnalités d’[!DNL Experience Manager].
