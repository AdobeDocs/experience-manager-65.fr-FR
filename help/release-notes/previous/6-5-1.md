...
* Le schéma de métadonnées contient uniquement des éléments de la configuration globale et ne contient pas ceux du client actif. La valeur de l’URL formPath est rétablie par défaut, même si elle est modifiée. NPR-29945 : correctif pour CQ-4262898
...
* Activation de la récupération de contenu prérendu à partir d’un point de terminaison distant. Correctif pour CQ-4270238
...
* Activation de la prise en charge d’ADFS v3.0 pour l’intégration Dynamics on-premise.
...
* Vérifiez que le composant ne consigne pas et n’affiche aucune erreur pour les modèles de document d’enregistrement. Correctif pour CQ-4272999
...
* Problèmes de performances lorsque l’application Forms adaptative est utilisée avec un jeu de données volumineux. Correctif pour CQ-4235310
...
* L’application Adaptive Forms a abandonné la prise en charge de Microsoft Windows 8.1. Correctif pour CQ-4265274
...
* Activation des options de préremplissage pour le canal d’impression de communication interactive dans la tâche Affecter . Correctif pour CQ-4265577
...
* La variable d’état de la signature de document ne prend pas en charge les variables. Correctif pour CQ-4266312
* Les envois depuis l’espace de travail échouent avec un caractère multipoint. Correctif pour CQ-4263172
...
* Lorsqu’une extension autre que xsd ou schema.json est téléchargée, le téléchargement ne se produit pas et aucun message d’erreur n’est généré. Correctif pour CQ-4266716
