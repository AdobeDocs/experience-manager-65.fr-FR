...
[!DNL Experience Manager] 6.5.16.0 comprend de nouvelles fonctionnalités, des améliorations importantes demandées par les clients, des correctifs de bogues, ainsi que des améliorations des performances, de la stabilité et de la sécurité, publiées depuis la version initiale de 6.5 en avril 2019. [Installez ce Service Pack](#install) sur [!DNL Experience Manager] 6.5. <!-- UPDATE FOR EACH NEW RELEASE -->
...
Nouveau protocole DASH (Dynamic Adaptive Streaming over HTTP) prend en charge la diffusion en continu à débit adaptatif dans les diffusions vidéo Dynamic Media (avec CMAF). [Format d’application de média commun] activée).
...
* L’interface utilisateur [!DNL Experience Manager] change pour la diffusion en continu DASH qui inclut les éléments suivants :
   * disposer d’un champ CMAF visible dans l’éditeur de profil vidéo.
...
>[!IMPORTANT]
>
> Adobe ne recommande pas de supprimer ou de désinstaller le module [!DNL Experience Manager] 6.5.16.0. Par conséquent, avant d’installer le module, vous devez créer une sauvegarde de la variable `crx-repository` au cas où vous deviez le restaurer. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Une requête GraphQL peut utiliser la variable `damAssetLucene` plutôt que l’index `fragments` index. Par conséquent, cela peut entraîner l’échec des requêtes GraphQL ou prendre un certain temps à s’exécuter.
...
* Sur la plateforme JBoss® 7.1.4, lorsqu’un utilisateur installe AEM Service Pack 6.5.16.0, `adobe-livecycle-jboss.ear` échec du déploiement.
