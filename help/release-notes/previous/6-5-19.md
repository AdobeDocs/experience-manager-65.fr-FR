...
[!DNL Experience Manager] 6.5.19.0 comprend de nouvelles fonctionnalités, des améliorations importantes demandées par la clientèle, des correctifs de bugs, ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiées depuis la version initiale 6.5 en avril 2019. [Installez ce pack de services](#install) dans [!DNL Experience Manager] 6.5.
...
* **[Améliorations de l’accessibilité dans AEM Forms Designer](https://helpx.adobe.com/content/dam/help/fr/experience-manager/6-5/forms/pdf/using-designer.pdf)** : AEM Forms Designer propose les améliorations d’accessibilité suivantes :

   * AEM Forms Designer prend désormais en charge la table des matières accessible par le biais de nouvelles balises `<TOC>` et `<TOCI>`, ce qui permet aux auteurs d’affecter ces rôles aux sous-formulaires via l’onglet Accessibilité . Cette fonctionnalité prend en charge des structures de table des matières à plusieurs niveaux, des entrées libellées, des références numérotées et des sous-structures étendues, ce qui permet aux lecteurs d’écran d’identifier et de parcourir correctement le contenu de la table des matières.

   * AEM Forms Designer prend désormais en charge la configuration des propriétés d’accessibilité des cellules de tableau, y compris la portée, l’étendue des lignes et l’étendue des colonnes, ce qui permet aux lecteurs d’écran et aux appareils d’assistance d’interpréter correctement la structure du tableau dans les PDF générés. Les auteurs peuvent définir ces propriétés directement à partir de la boîte de dialogue Propriétés de la cellule et les vérifier dans la vue XML Source. Les propriétés configurées sont reflétées avec précision dans la sortie PDF balisée et peuvent être validées à l’aide de l’éditeur de tableau Acrobat.
...
* Sur une page AEM Sites, lorsque vous effectuez un zoom avant de 200 %, les liens **[!UICONTROL Copie linguistique]** et **[!UICONTROL Rapport CSV]** dans le rail Références disparaissent. (SITES-11011)
...
#### Requêteur GraphQL{#sites-graphql-query-editor-6519}
...
* Lorsqu’un utilisateur ou une utilisatrice génère la sortie XML à l’aide du processus d’atelier, l’opération échoue pour certains fichiers. (FORMS-11314)
...
* Si des cellules fusionnées se trouvent dans le tableau, aucune interface d’utilisation graphique n’est disponible pour définir la plage (ligne et colonne) et la portée dans un tableau complexe du concepteur de formulaires AEM. (LC-3919532)
* Lorsqu’un utilisateur ou une utilisatrice tente de générer des documents PDF après avoir installé le package de modules complémentaires Forms sur le pack de services 6.5.15.0, la procédure échoue de manière intermittente et affiche l’erreur :
...
* La création d’une copie linguistique au niveau racine de la langue n’ajuste pas les chemins d’accès dans la page. Dans le cas où la copie linguistique a été créée, pas pour la racine de langue mais pour les pages qui la composent, le chemin d’accès a correctement changé. (NPR-41364)
...
* Lors de la création de la copie linguistique de la page « A », elle doit automatiquement créer les copies linguistiques des pages, des fragments d’expérience, des fragments de contenu et des ressources référencés. En outre, la nouvelle copie linguistique de la page « A » sur le nouveau chemin d’accès doit avoir ses références mises à jour sur les nouvelles copies linguistiques respectives des pages, des fragments d’expérience, des fragments de contenu et des ressources. (NPR-41076)
...
* [!DNL Experience Manager] 6.5.19.0 nécessite [!DNL Experience Manager] 6.5. Consultez la [documentation de mise à niveau](/help/sites-deploying/upgrade.md) pour obtenir des instructions détaillées. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!NOTE]
>
>Experience Manager 6.5.19.0 ne prend pas en charge l’installation Bootstrap. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Si vous effectuez une mise à niveau vers le pack de services AEM 6.5.15.0 ou une version ultérieure et que votre instance AEM fonctionne sur Tomcat 8.5.88, il est obligatoire d’installer le fragment de servlet *avant* de procéder à l’installation du pack de services 6.5.15.0 ou de toute version ultérieure.
...
* Les lignes du widget du sélecteur de date sont tronquées lors du parcours de plusieurs mois dans le widget de pop-up pour les champs suivant le modèle de modification/d’affichage. (FORMS-13620)
...
* [Liste des lots OSGi inclus dans Experience Manager 6.5.19.0](/help/release-notes/assets/65190_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
