...
[!DNL Experience Manager] 6.5.19.0 comprend de nouvelles fonctionnalités, des améliorations importantes demandées par les clients, des correctifs ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiés depuis la version initiale 6.5 en avril 2019. [Installez ce pack de services](#install) dans [!DNL Experience Manager] 6.5.
...
* Lorsque l’utilisateur tente de générer des documents PDF après l’installation du package complémentaire Forms sur AEM Forms Service Pack 6.5.15.0, l’erreur suivante s’affiche par intermittence :
...
* Sur la plateforme JBoss® 7.1.4, lorsque l’utilisateur ou l’utilisatrice installe le pack de services Experience Manager 6.5.16.0 ou version ultérieure, `adobe-livecycle-jboss.ear` déploiement échoue. (CQ-4351522, CQDOC-20159)
...
* Si vous effectuez une mise à niveau vers le pack de services AEM 6.5.15.0 ou supérieur et que votre instance AEM fonctionne sur Tomcat 8.5.88, il est obligatoire d’installer le fragment de servlet *avant* de procéder à l’installation du pack de services 6.5.15.0 ou supérieur.
...
* Après l’installation du pack de services AEM 6.5.14.0 et versions ultérieures, les utilisateurs ne peuvent pas sélectionner de police dans l’interface utilisateur d’administration JEE pour les documents PDF lors de la navigation vers `Home` > `Services` > `PDF Generator` > `Adobe PDF Settings`, car la liste des polices apparaît vide. (FORMS-12095)
...
* Lorsqu’un client enregistre un schéma de métadonnées comportant de nombreux champs, la requête multipart/form-data du créateur de formulaires dépasse la limite maxFileCount, ce qui entraîne des échecs silencieux. (CQDOC-22465)
