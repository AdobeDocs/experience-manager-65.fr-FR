---
title: Notes de mise à jour du pack de services [!DNL Adobe Experience Manager] 6.5
...
Adobe Experience Manager 6.5.5.0 est une mise à jour importante qui comprend de nouvelles fonctionnalités, des améliorations importantes demandées par les clients, ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiées depuis la mise à disposition de la version 6.5 en **avril 2019**. Elle peut être installée sur Adobe Experience Manager 6.5.

Voici quelques fonctionnalités et améliorations clés introduites dans [!DNL Adobe Experience Manager] 6.5.5.0 :
...
* Le référentiel intégré (Apache Jackrabbit Oak) a été mis à jour vers la version 1.22.3.

Voici la liste des correctifs fournis dans la version 6.5.5.0 d’[!DNL Experience Manager].
...
* Lorsque le nom d’un composant local dans une copie linguistique est identique au nom d’un composant dans le plan directeur et que ce composant est déployé à partir du plan directeur, le terme `_msm_moved` n’est pas ajouté au nom du composant local (NPR-33208).
...
>[!IMPORTANT]
>
>Il est conseillé aux utilisateurs de [!DNL Experience Manager desktop app] Windows d’effectuer une mise à niveau vers la version 2.0.3.2](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/release-notes.html?lang=fr#what-is-new) de l’appli de bureau [ pour accéder au référentiel de gestion des ressources numériques sur l’instance [!DNL Adobe Experience Manager 6.5.5.0]. Ils peuvent rencontrer des problèmes lors de l’accès au référentiel de gestion des ressources numériques sur [!DNL Adobe Experience Manager] instance 6.5.5.0 à l’aide de l’appli de bureau version 2.0.2.
...
* Des avertissements de traversée sont observés dans les journaux lors de l’ouverture d’une collection dynamique contenant plus de 10 000 ressources (NPR-32980).
...
* Les problèmes de performances résolus dans 6.5.5.0 sont les suivants (CQ-4279206) :
...
* Les groupes supprimés sur l’instance de création ne sont pas supprimés de toutes les instances de publication (NPR-33199).
...
UberJar pour Experience Manager 6.5.5.0 est disponible dans le [Référentiel Maven central](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.5/).
