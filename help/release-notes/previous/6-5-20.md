...
[!DNL Experience Manager] 6.5.20.0 comprend de nouvelles fonctionnalités, des améliorations importantes demandées par la clientèle, des correctifs de bugs, ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiées depuis la version initiale 6.5 en avril 2019. [Installez ce pack de services](#install) dans [!DNL Experience Manager] 6.5.
...
* Lorsque l’utilisateur sélectionne le champ de date doté d’un modèle d’affichage dans le pack de services 6.5.18.0 d’AEM Form, il ne peut pas sélectionner la date actuelle à l’aide du clavier. (FORMS-12736)
* Dans AEM Forms Service Pack 6.5.17.0 et Service Pack 6.5.18.0, lorsqu’un utilisateur ou une utilisatrice passe d’un mois à l’autre dans le widget de calendrier, le composant de sélecteur de date affiche une ligne supplémentaire. (FORMS-11869)
...
* Lorsque l’utilisateur effectue une mise à niveau vers le Service Pack 6.5.19.0 d’AEM Form, après la mise à jour d’une nouvelle langue dans le dictionnaire existant, elle n’est pas fusionnée avec les lignes « guideContainer » pour ajouter des paramètres régionaux à la page. (FORMS-12947)

* Sur le pack de services 6.5.19.0 d’AEM Forms, l’opération de service web appelée sur Java™ 11 échoue avec l’erreur (FORMS-12329) :
  `java.lang.NoClassDefFoundError message:sun/misc/BASE64Decoder`

* Lorsqu’un utilisateur appelle l’opération « receive » pour « EmailService » sur le pack de services 6.5.18.0 d’AEM Forms, une exception est générée (FORMS-12050) :
  `java.util.ServiceConfigurationError: javax.mail.Provider: Provider com.sun.mail.imap.IMAPProvider not a subtype`

* Lorsque le mode FIPS est activé sur le pack de services 6.5.18.0 d’AEM Forms, la création d’un utilisateur sous le DOM par défaut échoue avec l’erreur (FORMS-11857) :
...
* Sur le pack de services 6.5.18.0 d’AEM Forms, l’utilisateur ne peut pas créer de paramètres de sécurité, il n’affiche aucun journal d’erreur ou de serveur, mais un message d’erreur pop-up s’affiche à l’écran. (FORMS-12212)

* Lorsque l’utilisateur du pack de services AEM Forms 6.5.18.0 envoie un formulaire adaptatif au workflow JEE, la pièce jointe du formulaire adaptatif n’est pas envoyée au processus JEE, ce qui entraîne l’échec de l’application. (FORMS-12232, FORMS-12228)
...
* Sur AEM Forms 6.5.18.0, lorsqu’un formulaire adaptatif est publié, toutes ses dépendances, y compris les politiques, sont republiées, même si aucune modification n’y a été apportée. (FORMS-10454)

* Lorsqu’un utilisateur sélectionne « Microsoft® SharePoint » lors de l’exécution du gestionnaire de configuration sur AEM Forms 6.5.19.1 avec la configuration clé en main de JBoss®, l’installation de LiveCycle JBoss® EAR échoue et le message d’erreur suivant apparaît : (FORMS-12463)
...
* Lorsque l’utilisateur crée un fragment de document à l’aide du modèle de données de formulaire dans le pack de services 6.5.19.0 d’AEM Forms, les noms des variables apparaissent non définis dans le panneau latéral. Toutefois, les noms des variables s’affichent lorsque vous les faites glisser sur le panneau du formulaire ou lorsque vous cliquez dessus. (FORMS-13238)


#### [!DNL Forms Designer] {#forms-designer-6520}


* Lorsqu’un utilisateur ou une utilisatrice effectue une mise à niveau vers le pack de services AEM Forms 6.5.18.0, en raison d’une gestion des exceptions manquante, les XDP transmis par le service de sortie avec l’option PDF balisée activée échouent. (LC-3921757)
...
* Les liens dans les fragments d’expérience ne sont pas adaptés à la bonne langue lors de la copie de la langue. Au lieu de cela, le fragment d’expérience pointe vers les paramètres régionaux principaux. (NPR-41343)
...
* [!DNL Experience Manager] 6.5.20.0 nécessite [!DNL Experience Manager] 6.5. Consultez la [documentation de mise à niveau](/help/sites-deploying/upgrade.md) pour des informations détaillées. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!NOTE]
>
>Experience Manager 6.5.20.0 ne prend pas en charge l’installation Bootstrap. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Après la mise à jour d’AEM 6.5 Forms Service Pack 18 (6.5.18.0) ou d’AEM 6.5 Forms Service Pack 19 (6.5.19.0) vers AEM 6.5 Forms Service Pack 20 (6.5.20.0), les utilisateurs et utilisatrices rencontrent une erreur de compilation JSP. Il est impossible d’ouvrir ou de créer des formulaires adaptatifs et des erreurs se produisent avec d’autres interfaces d’AEM telles que l’éditeur de page, l’interface d’utilisation d’AEM Forms et l’éditeur de workflow d’AEM. Vous pouvez installer la [dernière version d’AEM 6.5 Form Service Pack 20 (6.5.20.0)](https://experienceleague.adobe.com/fr/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases) pour résoudre le problème. (FORMS-13492)
* Dans un formulaire adaptatif basé sur un XDP avec des scripts intégrés sur des cases à cocher, les scripts ne sont pas exécutés pour les éléments après ces cases à cocher. (FORMS-14244)
* Les lignes du widget du sélecteur de date sont tronquées lors du parcours de plusieurs mois dans le widget de pop-up pour les champs suivant le modèle d’édition/d’affichage. (FORMS-13620)
* Les envois de formulaire échouent lors de la tentative d’utilisation du service DOR (Document d’enregistrement) dans le serveur principal. Le message d’erreur rencontré est : « L’action Envoyer n’a pas pu se terminer, car la ressource de formulaire n’a pas été correctement affectée. » (FORMS-13798)
