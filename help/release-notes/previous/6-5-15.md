...
* Le texte du filtre n’a pas de rapport de contrat de 4,5 :1 par rapport à la couleur d’arrière-plan. (ASSETS-6873)
* Le texte du nom du dossier sur la page de navigation principale n’a pas de rapport de contraste de 4,5 :1 par rapport à la couleur d’arrière-plan. (ASSETS-6872)
...
* Les options Portée, Workflows et Fuseau horaire n’ont pas de rapport de contraste de 4,5 :1 par rapport à la couleur d’arrière-plan. (ASSETS-6868)
...
* Le nombre d’erreurs de traitement par lots de l’avis est démultiplié après la mise à niveau du pack de services 6.5.11.0. (NPR-39636)
...
* Le texte du lien ou du bouton n’a pas de rapport de contraste de 4,5 :1 au survol ou à la sélection. (SITES-8756, SITES-8885)
...
* [!DNL Experience Manager] 6.5.15.0 nécessite [!DNL Experience Manager] 6.5. Consultez la [documentation de mise à niveau](/help/sites-deploying/upgrade.md) pour des informations détaillées. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* Le Service Pack d’AEM Forms JEE 6.5.15.0 n’est pas installé correctement dans l’environnement Linux JBoss. Validez l’installation correcte du Service Pack d’6.5.15.0 AEM Forms JEE en vérifiant le fichier `PatchInstallerProcessing[1-9*].log` pour l’entrée de journal, `[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/xml/RUP_BOM.xml not found! Assuming this component is not in the installation. Skipping Processing`. Cette entrée indique l’échec de l’installation du pack de services 6.5.15.0 d’AEM Forms JEE. Pour résoudre le problème d’installation avec le Service Pack d’AEM Forms JEE 6.5.15.0, [ajoutez manuellement le fichier RUP_BOM.xml](/help/forms/using/linux-jboss-installation-issue-on-sp15.md) dans le répertoire XML.

* L’installation du pack de services réinitialise la propriété de langues du nœud /content/cq:tags sur la valeur par défaut. Il est donc nécessaire de l’ajouter à partir des propriétés avant l’installation.
