...
* **Formulaires adaptatifs**
   ...
   * Lorsque l’utilisateur tente de créer un formulaire adaptatif à l’aide d’un schéma JSON complexe, l’opération échoue. L’erreur se produit comme suit :
...
* [!DNL Experience Manager] 6.5.18.0 nécessite [!DNL Experience Manager] 6.5. Consultez la [documentation de mise à niveau](/help/sites-deploying/upgrade.md) pour obtenir des instructions détaillées. <!-- UPDATE FOR EACH NEW RELEASE -->
...
1. Le bundle OSGi `org.apache.jackrabbit.oak-core` est de la version 1.22.16 ou ultérieure (utiliser la console web : `/system/console/bundles`).
...
* JDK 11.0.20 n’est pas pris en charge pour installer le programme d’installation d’AEM Forms sur JEE. Seules les versions JDK 11.0.19 ou antérieures sont prises en charge pour installer le programme d’installation d’AEM Forms sur JEE. (FORMS-10659)
...
Pour résoudre ce problème, recherchez le fichier `<AEM_Forms_Installation_dir>\jboss\bin\standalone.bat` et mettez à jour `Adobe_Adobe_JAVA_HOME` pour `Adobe_JAVA_HOME` toutes les occurrences avant d’exécuter le gestionnaire de configuration. (CQDOC-20803)
