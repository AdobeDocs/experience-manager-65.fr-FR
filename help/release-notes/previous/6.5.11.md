...
description: "Notes relatives à [!DNL Adobe Experience Manager]\_6.5 décrivant les informations de version, les nouveautés, la procédure d’installation et les listes de modifications détaillées."
...
* Lorsqu’un utilisateur du groupe de contributeurs accède au référentiel de ressources de gestion des ressources numériques, une requête `POST` exceptionnelle est déclenchée pour créer une collection. Cette requête `POST` échoue et reflète une erreur dans les journaux (NPR-37171).
...
* Lorsque l’utilisateur tente de convertir des formulaires XDP du PDF au HTML5. Voici quelques problèmes qui se produisent lors du rendu des formulaires HTML5 pour les polices cambodgiennes :
   * Les sauts de ligne sont générés dans les formulaires HTML5. (CQ-4322508)
   * Les listes ne s’affichent pas correctement dans les formulaires HTML5. (CQ-4322317)
   * Des espaces supplémentaires s’affichent lors du rendu de HTML5 forms. (CQ-4322509)
...
>[!NOTE]
>
>Si vous n’avez pas effectué la mise à niveau vers AEM Forms 6.5.11.0, installez directement le module complémentaire AEM Forms 6.5.11.1. Si vous avez installé AEM Forms 6.5.11.0, Adobe recommande d’effectuer la mise à niveau vers AEM Forms 6.5.11.1.

* Les actions Envoyer, Envoyer un e-mail et Appeler un workflow AEM ne fonctionnent plus après l’installation du module complémentaire Forms 6.5.11.0.
* L’opération Créer un PDF cesse de convertir des documents Microsoft Word en documents PDF après l’installation du module complémentaire Forms 6.5.11.0.
...
* Lorsque l’utilisateur tente de convertir des formulaires XDP de PDF en HTML5, certains des problèmes résolus sont les suivants :
   * Prise en charge du rendu du farsi en tant que LTR.
   * Prise en charge de la durée de vie (RTL) lorsqu’elle est mélangée avec du texte LTR.
   * Prise en charge de la rotation des images.
   * La prise en charge de la langue cambodgienne est ajoutée. Pour activer la prise en charge Combodian, vous devez activer l’indicateur comme `CTL=true` lors du rendu du formulaire HTML5.
...
* Lors d’un déploiement avec MongoDB et plusieurs instances, installez Experience Manager 6.5.11.0 sur l’une des instances d’auteur à l’aide du gestionnaire de modules.
...
1. Ouvrez le gestionnaire de modules et cliquez sur **[!UICONTROL Télécharger le package]** pour télécharger le package. Pour en savoir plus, consultez la section [Gestionnaire de modules](/help/sites-administering/package-manager.md).
...
B. Utilisez l’[API HTTP à partir du gestionnaire de modules](/help/sites-administering/package-manager.md#package-share). Utilisez `cmd=install&recursive=true` afin que les modules imbriqués soient installés.
