...
* Problèmes liés au traitement des ressources de PDF en cas de référentiels volumineux (CQ-4328711).

* L’erreur de traitement du PDF ne se propage pas vers [!DNL Experience Manager] en cas d’échec au niveau de [!DNL Scene7] (CQ-4331145).
...
* S’il existe un workflow de conteneur incorporé, vous recevez plusieurs emails d’achèvement de workflow même après avoir sélectionné la variable `Notify on Complete of Container Workflow` (NPR-37280).
...
>[!NOTE]
>
>Si vous n’avez pas effectué la mise à niveau vers AEM Forms 6.5.11.0, installez directement le package complémentaire AEM Forms 6.5.11.1. Si vous avez installé AEM 6.5.11.0 Forms, Adobe vous recommande de mettre à niveau vers AEM 6.5.11.1 Forms.
...
* Si vous utilisez des fragments de contenu et GraphQL, il est recommandé de mettre à niveau vers la version 6.5.12.0 et d’installer les packages appropriés.
   * Si la mise à niveau vers la version 6.5.12.0 n’est pas une option, contactez le support pour évaluer si une solution peut être mise en oeuvre entre-temps.
...
* Lorsque vous tentez de déplacer/supprimer/publier des fragments de contenu ou des sites/pages, un problème se produit lorsque les références aux fragments de contenu sont récupérées, car la requête d’arrière-plan échoue. En d’autres termes, la fonctionnalité ne fonctionnera pas.
