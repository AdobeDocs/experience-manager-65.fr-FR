...
* Vous pouvez désormais utiliser [!DNL Dynamic Media] pour configurer les paramètres généraux au lieu d’avoir à passer par l’ [!DNL Dynamic Media Classic] application de bureau. Voir [Configurer les paramètres généraux de Dynamic Media](/help/assets/dm-general-settings.md).
...
* Si vous utilisez des fragments de contenu et GraphQL, il est recommandé de mettre à niveau vers la version 6.5.12.0 et d’installer les packages appropriés.
   * Si la mise à niveau vers la version 6.5.12.0 n’est pas une option, contactez le support pour évaluer si une solution peut être mise en oeuvre en attendant.
