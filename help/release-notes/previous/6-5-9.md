...
[!DNL Adobe Experience Manager] 6.5.9.0 comprend de nouvelles fonctionnalités, des améliorations importantes demandées par les clientes et clients, ainsi que des améliorations en termes de performances, de stabilité et de sécurité, publiées depuis la version 6.5 d’avril 2019. Le pack de services est installé sur [!DNL Adobe Experience Manager] 6.5.

Les principales fonctionnalités et améliorations introduites dans [!DNL Adobe Experience Manager] 6.5.9.0 sont les suivantes :

* Le composant Dynamic Media Foundation d’[!DNL Experience Manager Sites] vous permet désormais d’activer ou de désactiver l’optimisation pour les appareils à résolution plus élevée lors de l’utilisation d’un paramètre d’image prédéfini réactif ou d’un recadrage intelligent.
...
* Les occurrences de noms liés à Hong Kong, Macao et Taïwan sont mises à jour conformément aux nouvelles conventions de dénomination pour les zones géographiques et les paramètres régionaux chinoises.
...
* Le référentiel intégré (Apache Jackrabbit Oak) a été mis à jour vers la version 1.22.7.
...
Voici la liste des correctifs fournis dans la version 6.5.9.0 d’[!DNL Experience Manager].
...
* Une requête de traversée est déclenchée lors de l’utilisation de l’API `com.day.cq.wcm.commons.ReferenceSearch`. Cela affecte les performances du [!DNL Experience Manager] (NPR-36407).
...
* Les propriétés de déploiement sur page de Multi-Site Manager du gabarit de page ignorent la case à cocher Déployer la page et les sous-pages (NPR-35634).
...
* Lorsque vous ouvrez la boîte de dialogue pour ajouter des ressources à l’aide des touches du clavier dans l’éditeur [!UICONTROL Ensemble d’images] :
...
* Le contraste du texte (avec l’arrière-plan) et des contrôles sur la page de l’éditeur d’ensemble d’images est amélioré, de sorte que les personnes ayant une vision et une perception limitées des couleurs puissent mieux comprendre le contenu de l’interface (CQ-4290733).
...
* Lors de la navigation dans la page Modifier le paramètre d’image prédéfini en mode Formulaire, le lecteur d’écran annonce le rôle et le nom des différents champs et contrôles (CQ-4290717).
...
* L’astérisque visuel permettant de désigner le champ obligatoire est désormais fourni dans le champ Titre de la ressource dans l’éditeur d’ensemble d’images, et les lecteurs d’écran annoncent les informations requises pour le champ (CQ-4290712).
...
* Lorsque vous créez un dossier, que vous sélectionnez la propriété Organisable, puis que vous ajoutez plus de 20 ressources au dossier, la sélection de toutes les ressources du dossier affiche un nombre incorrect (CQ-4316243).
...
* Les lots personnalisés ne sont pas mis à jour lors de l’installation d’un nouveau package de code à l’aide du gestionnaire de modules (NPR-35949).
...
UberJar pour Experience Manager 6.5.9.0 est disponible dans le [Référentiel Maven central](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.9/).
