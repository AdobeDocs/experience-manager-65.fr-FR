...
* **FORMS-23802** (les fonctions personnalisées ne se chargent pas lorsque le formulaire se trouve dans la page Sites) : les fonctions personnalisées ne fonctionnent pas dans l’aperçu ou la publication lorsque le formulaire adaptatif est incorporé dans une page Sites et que la version de la bibliothèque de composants principaux aem-forms est inférieure à 1.1.76. Il se peut qu’une erreur telle que `InvalidFormContainerException: No form container found` s’affiche dans les journaux. Pour résoudre ce problème, [télécharger et installer le correctif](/help/release-notes/aem-forms-hotfix.md) pour AEM Forms SP24 (AddOn 6.0.1454).
