...
* [hCaptcha](/help/forms/using/integrate-adaptive-forms-hcaptcha.md) et [Services Cloudflare Turnstile CAPTCHA](/help/forms/using/integrate-adaptive-forms-turnstile.md) : AEM Forms prend en charge les services Captcha suivants :
...
* Contrôle de version de formulaire adaptatif :
   * [Créer plusieurs versions d’un formulaire adaptatif](/help/forms/using/add-versioning-reviews-comments.md) - Désormais, les utilisateurs peuvent facilement gérer des variantes de formulaires existants. Ce processus simplifie le contrôle des versions et facilite la comparaison pour l’optimisation des formulaires, le tout au sein d’un seul processus simplifié.
...
* Ajout de la prise en charge de l’incorporation des polices dans les [API de lot de communications interactives](https://experienceleague.adobe.com/fr/docs/experience-manager-65/content/forms/interactive-communications/create-interactive-communication#output-format-print-channel) - Les communications interactives incluent désormais la prise en charge de l’incorporation des polices Adobe Ming et Adobe Myungjo dans les PDF générés via l’API de lot. Cette amélioration garantit un rendu de texte précis dans les documents générés, même lors de l’utilisation de sous-ensembles de polices, offrant ainsi une meilleure prise en charge du contenu multilingue dans les sorties du PDF.

* [API Table of Content pour l’accessibilité du PDF](/help/forms/using/aem-document-services-programmatically.md#auto-tag-pdf-documents-auto-tag-api) - AEM Forms sur OSGi prend désormais en charge la nouvelle API TOC Tag pour améliorer le PDF pour les normes d’accessibilité. Cela rend les PDF plus accessibles pour les utilisateurs et utilisatrices utilisant des technologies d’assistance.

* [Résolution XDP de fragment](/help/forms/using/assembler-service.md#resolve-references-on-crx-repository-resolve-references-on-crx-repository) - AEM Forms sur OSGi résout désormais les fichiers XDP de fragment référencés dans les fichiers XDP de Principal et stockés dans le référentiel CRX d’AEM.

* [Améliorations de la conformité PDF/A](/help/forms/developing/pdf-a-documents.md#converting-documents-to-pdfa-documents-converting-documents-to-pdf-a-documents) - Désormais, les utilisateurs peuvent convertir des PDF au format PDF/A (1a, 2a, 3a) à des fins d’archivage tout en assurant l’accessibilité et en vérifiant la conformité avec ces normes.

* **Prise en charge du dimensionnement automatique des polices pour les documents de PDF statique** - AEM Forms Designer, OutputService et FormsService prend désormais en charge le dimensionnement automatique des polices pour les PDF statiques. Si l’utilisateur définit la taille de police 0 pour les champs de texte, numériques, de mot de passe ou de date et heure, elle s’ajuste automatiquement dans ces champs sans modifier la taille globale du champ. Pour utiliser la fonctionnalité, les utilisateurs transmettent un indicateur dans la XCI personnalisée : `<behaviorOverride>patch-LC-3921991:1</behaviorOverride>`.
...
* Lorsqu’un utilisateur d’AEM Forms Service Pack 19 (6.5.19.0) prévisualise une lettre, le contenu ne s’aligne pas correctement, car les espaces apparaissent manquants et le caractère `x` apparaît à certains emplacements. (FORMS-16670)
...
* Lorsqu’un utilisateur effectue la mise à niveau d’AEM Forms Service Pack 17 (6.5.17.0) vers AEM Forms Service Pack 21 (6.5.21.0), le chemin d’URL d’envoi du formulaire adaptatif modifié ne fonctionne pas. (FORMS15894)
...
* Lorsqu’un utilisateur utilise la fonctionnalité d’affichage/de masquage d’un tableau dans un formulaire adaptatif, la taille du champ se réduit. La taille du champ se corrige lors de l’ajout et de la suppression d’une ligne. (FORMS-14756)
...
* Lorsqu’un utilisateur effectue une mise à jour d’AEM Forms Service Pack 15 (6.5.15.0) vers Service Pack 19 (6.5.19.0), un problème se produit. Un modèle d’affichage personnalisé défini sur `num{$zzz,zz9.99}` ne s’affiche pas correctement dans l’aperçu et l’interface utilisateur de l’agent. (FORMS-14694)
...
* Dans AEM Forms Service Pack 20 (6.5.20.0), les utilisateurs qui envoient des courriers électroniques avec des pièces jointes à l’aide du bouton &quot;Envoyer un courrier électronique&quot; dans les formulaires adaptatifs rencontrent un problème. Le nom de la pièce jointe apparaît sur la ligne suivante plutôt que sur la ligne d’entrée. (FORMS-14426)
* Lorsqu’un utilisateur génère un PDF dans AEM Forms avec des listes à puces définies sur le style &quot;Disque&quot; par défaut, le PDF échoue le contrôle de l’accessibilité dans Adobe Acrobat et  l’outil d’accessibilité. Les styles Liste avec &quot;Puce&quot; et &quot;Carré&quot; transmettent la vérification de l’accessibilité. (FORMS-13802, LC-3922179)
...
* Dans AEM Forms on JEE, dans AEM Workspace, la sélection d’un formulaire précédemment envoyé pour démarrer un nouveau processus de formulaire entraîne un problème. Les données Forms préremplies remplacent toutes les données précédemment envoyées, supprimant ainsi les champs remplis manuellement. (FORMS-15376)
...
* Mise à niveau dans les fichiers jar d’AEM Forms sur JEE : la bibliothèque `commons-collections:commons-collections:jar` est désormais incluse afin d’améliorer la résolution des dépendances et les fonctionnalités des différents traitements d’AEM Forms JEE, comme suit :
   ...
   * Amélioration de la tâche du Rights Management pour sécuriser la gestion des documents et améliorer les fonctionnalités du Rights Management.
