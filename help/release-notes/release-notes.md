...
* **Prise en charge de la transmission de XCI personnalisés :** Ajout de la prise en charge de la transmission de XCI personnalisés dans les paramètres de l’application cmdline xmlformcmd. Cette fonctionnalité permet aux utilisateurs de spécifier des fichiers XCI personnalisés à des fins de test, améliorant ainsi la flexibilité et le contrôle sur le processus de test. (LC-3923248)
...
* Les lecteurs d’écran reçoivent un message `Loading` parlé lors de l’actualisation de la vue Sites. La mise à jour ajoute une région active de statut dédiée et y écrit le message par programmation, ce qui confirme la progression sans déplacer le focus. (SITES-24481)
...
* La boîte de dialogue Image annonce désormais son état de chargement via une région active ARIA. Les lecteurs d’écran entendent un message `Loading, please wait` lorsque la double flèche s’affiche. Et une mise à jour prête lorsque le contenu se termine, afin que les utilisateurs sachent quand ils peuvent interagir. (SITES-24697)
* La boîte de dialogue Sélection de lien expose désormais une région active qui annonce les résultats de la recherche. Les lecteurs d’écran entendent le statut `results updated` après chaque recherche sans déplacer le focus, de sorte que les utilisateurs reçoivent une confirmation claire que la recherche est terminée. (SITES-24700)
...
* Modifier les filtres indique désormais que son statut est développé ou réduit. Le bouton bascule en `aria-expanded` synchronisé avec le panneau de filtrage et expose un seul nom clair (`Change filters`), supprimant l’annonce `filter?` déroutante. Les utilisateurs de lecteurs d’écran peuvent prédire le résultat de l’activation du contrôle. (SITES-24713)
...
* Les mises à jour de statut dans la vue Filtre de l’administrateur de sites sont annoncées aux lecteurs d’écran. Lorsque les utilisateurs changent de carte/liste pendant le chargement des éléments, le NVDA diffuse désormais le message `Please wait` dans une zone géographique active. Ces conseils évitent les clics supplémentaires et la confusion. (SITES-24992)
...
* L’action de confirmation dans la boîte de dialogue Teaser s’affiche correctement dans les lecteurs d’écran. Les rapports de contrôle ne `Confirm` pas la description des icônes, donnant des conseils clairs aux utilisateurs du clavier et des lecteurs d’écran. (SITES-25223)
* Le bouton Aide propose désormais un nom accessible et clair. Les lecteurs d’écran annoncent `Help` au lieu d’une description d’icône détaillée. Les utilisateurs comprennent l’action et peuvent obtenir de l’aide plus rapidement. (SITES-25224)
...
* Les lecteurs d’écran annoncent les actions correctes pour les boutons **Annoter** et **Fermer l’annotation**. Le NVDA ne dit plus `Preview button pressed` ; il annonce le nom du bouton afin que les utilisateurs sachent quand le mode d’annotation commence ou se termine. (SITES-25268)
...
* Le bouton Modifier annonce désormais l’action correcte aux lecteurs d’écran dans la barre d’outils secondaire. Son activation indique `Edit` au lieu de la `Preview button pressed,` sans rapport, ce qui supprime la confusion lors de la navigation au clavier. (SITES-25320)
* Le curseur de panier de la barre d’outils Démographie expose désormais un nom accessible approprié. Les lecteurs d’écran annoncent que les outils de `Cart total` et de saisie vocale peuvent cibler le contrôle par nom, améliorant ainsi la conformité au WCAG 4.1.2 (Nom, Rôle, Valeur). (SITES-25322)
...
* AEM annonce l’état de chargement dans la boîte de dialogue modale Teaser . La boîte de dialogue modale expose désormais un message de statut actif lors du chargement du contenu. De ce fait, NVDA et JAWS parlent `Loading, please wait.` les auteurs doivent recevoir des commentaires clairs et éviter d’interagir avec la boîte de dialogue avant qu’elle ne soit prête. (SITES-25366)
...
* Le repère du rail Références annonce désormais son nom aux lecteurs d’écran. La région expose un `aria-label` unique (`References rail`), ce qui améliore la navigation et la distingue des autres régions. (SITES-24973)
...
* Il manquait un libellé spécifique à la liste déroulante Catégories. JAWS parlait donc un `images button menu.` générique. La mise à jour nomme le `Categories` de contrôle et définit son rôle. Les utilisateurs de lecteurs d’écran entendent un libellé précis et comprennent les choix disponibles. (SITES-35226)
...
* Le champ multiple utilisé dans un composant personnalisé exposait des boutons d’icône sans libellé et un ordre de tabulation incohérent. JAWS/NVDA a uniquement annoncé la `button` ou l’omission de commandes, ce qui a bloqué l’utilisation du clavier. La mise à jour fournit des noms descriptifs pour Ajouter, Supprimer et Déplacer, normalise les taquets de tabulation et annonce les mises à jour de liste pour répondre aux attentes d’ADA. (SITES-30660)
...
Les parenthèses dans un nom de fragment de contenu entraînaient une utilisation incorrecte du panneau Références. Les auteurs ont vu 0 même lorsque d’autres fragments l’ont référencé. Le correctif corrige l’analyse du chemin d’accès pour `(` et `)` et surfacie le nombre correct de valeurs non nulles et les entrées. (SITES-35078)
...
* Échec des libellés de balise personnalisée dans l’éditeur de page et l’interface utilisateur toujours affichée `Tags.` Le prédicat évalue désormais `fieldLabel` première et `labelText` seconde, puis applique la valeur par défaut. Les auteurs voient le libellé qu’ils ont défini. (SITES-32278)
...
* Les utilisateurs rencontraient des problèmes d’accessibilité dans DoR PDF forms où le NVDA annonçait incorrectement des `unavailable` pour les cases à cocher, les boutons radio et les champs de texte. Le message a souvent été répété, ce qui a semé la confusion chez les utilisateurs de lecteurs d’écran. (LC-3923201)
...
* Une page principale en espagnol vierge s’affichait lorsque seul le contenu en anglais était sélectionné dans un XDP (XML Data Package) avec plusieurs pages principales. (LC-3923009)
* Les utilisateurs ont observé des informations obsolètes sur l’année de copyright dans AEM Designer. Cette erreur s’est produite dans la zone pop-up au démarrage, dans la section `About` et dans la section `Legal Notices`, affichant `2003-2024` au lieu de « 2003-2025 ». (LC-3923005)
* Les utilisateurs ont rencontré une page PDF vierge lors de l’utilisation de la pagination dans AEM Forms Designer. Le problème s’est produit lors de la sélection de `Top of the Next Page/Top of Page` pour WireAdviceHeader, ce qui a perturbé la mise en page des itérations de données. (LC-3922997, LC-3922830)
* Les utilisateurs ont rencontré un problème en raison duquel la valeur de résumé de fichier pour la définition de schéma (XSD) XML (Extensible Markup Language) n’était pas conservée dans la version 64 bits d’AEM Forms Designer. (LC-3922924)
...
* Les utilisateurs ont rencontré une erreur avec la couleur d’arrière-plan du `Dati Richiedente` d’en-tête dans Composant d’application portable (PAC), recevant le `path object not tagged.` de message (LC-3922912)
...
* Les utilisateurs ont rencontré des problèmes lors de la création de règles pour afficher un panneau et définir la cible d’action sur un élément à l’intérieur. La règle setFocus est exécutée avant la mise à jour de visibilité, ce qui entraîne l’échec de l’action de focus. (FORMS-19563)
...
* Les utilisateurs ont rencontré une erreur `Invalid URL` lors de l’annotation de ressources dans AEM 6.5.22. (NPR-42684)
...
* L’icône d’aide expose désormais un libellé concis aux technologies d’assistance. JAWS lit les `Help button` et n’ajoute plus de termes `menu` superflus. Cette mise à jour apporte le contrôle à la conformité WCAG 4.1.2 et simplifie l’utilisation du clavier et du lecteur d’écran. (GRANITE-55360)
...
* Correction du texte d’info-bulle tronqué dans le panneau Image. La description `Display caption as pop-up` s’affiche entièrement dans tous les paramètres régionaux pris en charge, ce qui améliore le guidage pour les auteurs non anglophones. (SITES-10490)
* L’affichage Colonne d’administration de sites a tronqué les libellés localisés en français et en espagnol. `End Time` et `Off Time` sont apparus tronqués et n’ont affiché aucune info-bulle. Adobe a corrigé les traductions et restauré l’info-bulle au survol, de sorte que les libellés soient lus en entier. (SITES-31318)
* La boîte de dialogue **Déplacer** dans Sites affichait des clés i18n brutes au lieu de libellés lisibles. Des éléments tels que `Referencing pages,` `Created on,` `Created by,` et `Path` semblaient confus. Le correctif relie la boîte de dialogue aux dictionnaires corrects et fournit les traductions, avec un secours en anglais. (SITES-30881)
...
* [!DNL Experience Manager] 6.5.24.0 nécessite [!DNL Experience Manager] 6.5. Consultez la [documentation de mise à niveau](/help/sites-deploying/upgrade.md) pour obtenir des instructions détaillées. <!-- UPDATE FOR EACH NEW RELEASE -->
...
>[!IMPORTANT]
>
> Adobe ne recommande pas de supprimer ou de désinstaller le package [!DNL Experience Manager] 6.5.24.0. Par conséquent, avant d’installer le module, vous devez créer une sauvegarde du `crx-repository` au cas où vous auriez besoin de le restaurer. <!-- UPDATE FOR EACH NEW RELEASE -->
...
Le LTS SP2 d’AEM 6.5 fournit des API OpenAPI modernes pour la gestion des fragments de contenu et des modèles, de sorte que les anciens points d’entrée de la prise en charge des fragments de contenu dans l’API REST d’AEM Assets sont désormais obsolètes.
...
* **FORMS-15681** Lorsque la lettre est affichée en aperçu avant impression, le contenu est modifié. Certains espaces disparaissent et certaines lettres sont remplacées par `x`.
...
* [Liste des lots OSGi inclus dans Experience Manager 6.5.24.0](/help/release-notes/assets/65240-bundles.txt)
<!-- UPDATE FOR EACH NEW RELEASE -->
* [Liste des modules de contenu inclus dans Experience Manager 6.5.24.0](/help/release-notes/assets/65240-packages.txt)
<!-- UPDATE FOR EACH NEW RELEASE -->
