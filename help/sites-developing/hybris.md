...
>[!NOTE]
>
>La structure eCommerce peut être utilisée avec n’importe quelle solution eCommerce. Certain specifics and examples dealt with here will refer to the [SAP Commerce Cloud](https://www.hybris.com/) solution.
...
* La propriété `cq:commerceProvider` est également utilisée pour référencer la définition de fabrique de commerce appropriée.
   ...
   * D’autres propriétés, telles que [**Version du catalogue **, peuvent être configurées ici](#configuringthecatalogversion), le cas échéant.
...
* Lors de l’appel de maven, ajoutez l’argument de ligne de commande suivant à la commande
   `-P hybris4`Il télécharge la distribution Hybris 4 préconfigurée et l’incorpore dans le lot :
...
* Authentification AEM est supposé être le *seul* système frontal web et réalise donc l’*ensemble* de l’authentification.

* Comptes esclaves AEM crée un compte esclave dans hybris pour chaque client. Le nom d’utilisateur du compte esclave est identique au nom d’utilisateur AEM. Un mot de passe aléatoire sur le plan cryptographique est généré automatiquement et stocké (chiffré) dans AEM.
