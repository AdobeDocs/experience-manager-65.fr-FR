...
* Sur l’instance de publication, les composants affichent votre contenu, le présentant selon vos besoins aux visiteurs de votre site web.
...
>[!NOTE]
>
>Il est également possible de redéfinir les composants avec une superposition en fonction de la logique du chemin de recherche. Toutefois, dans ce cas, la variable [Sling Resource Merger](/help/sites-developing/sling-resource-merger.md) n’est pas déclenché et `/apps` doit définir l’intégralité de la superposition.
...
[IU Coral](https://developer.adobe.com/experience-manager/reference-materials/6-5/coral-ui/coralui3/index.html) et [IU Granite](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/index.html) définir l&#39;aspect moderne de l&#39;AEM.

[L’IU Granite offre un vaste éventail de composants de base (widgets)](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/index.html) nécessaires pour créer une boîte de dialogue dans l’environnement de création. Au besoin, vous pouvez étendre cette sélection et [créer votre propre widget](#creatinganewwidget).
...
>Voir :
>
>* la session AEM Gems sur [Personnalisation des champs de boîte de dialogue](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html?lang=en).
...
>[!NOTE]
>
>Pour plus d’informations sur l’interface utilisateur Granite, reportez-vous à la section [Documentation de l’interface utilisateur Granite](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/index.html).
...
La méthode de gestion des événements dans les champs de boîte de dialogue est désormais appliquée avec les [écouteurs d’une bibliothèque cliente personnalisée](#listeners-in-a-custom-client-library). Il s’agit d’un changement par rapport à l’ancienne méthode de [écouteurs dans la structure de contenu](#listenersinthecontentstructureclassicui).
...
AEM offre la possibilité de configurer un système de paragraphes sur votre page afin que [une instance de votre nouveau composant est automatiquement créée lorsqu’un utilisateur fait glisser une ressource (appropriée) sur une instance de cette page.](/help/sites-authoring/editing-content.md#insertingacomponenttouchoptimizedui) (au lieu d’avoir toujours à faire glisser un composant vide sur la page).
...
* Boîtes de dialogue
   ...
   * Personnalisation des champs, pour plus d’informations, consultez la session AEM Gems sur [Personnalisation des champs de boîte de dialogue](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html?lang=en).
   * Migration des types vers [Validation de l’IU Granite](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/clientlibs/foundation/js/validation/index.html)
...
Pour cette raison, il est facile de rendre tout balisage de documentation existant disponible dans le composant lui-même.

Placez un `README.md` dans la structure du composant. Cette balise s’affiche dans la variable [console de composants](/help/sites-authoring/default-components-console.md).
