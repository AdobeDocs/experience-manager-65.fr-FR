...
Cela dépend de votre emplacement et affiche les outils permettant de contrôler la vue ou les ressources dans la page ci-dessous. La barre d’outils est spécifique au produit, mais les éléments ont un aspect commun.
...
Le cadre d’éditeur réalise les fonctions d’édition.
...
Dans l’onglet Ressource , vous pouvez utiliser une liste déroulante pour sélectionner les groupes de ressources spécifiques.
...
Ces recouvrements recouvrent le cadre de contenu et sont utilisés par la variable [calques](#layer) pour réaliser le mécanisme d’interaction (de manière transparente) avec les composants et leur contenu.
...
Chaque instance d’un composant affiche sa barre d’outils en cas de clic (une fois ou avec un double-clic lent). La barre d’outils contient les actions spécifiques (par exemple, copier, coller, ouvrir l’éditeur) disponibles pour l’instance de composant (Modifiable) sur la page.
