...
Grâce aux modèles modifiables, les tâches de création et de gestion des modèles ne sont plus réservées aux seuls développeurs. Un type d’utilisateur expérimenté, appelé créateur (ou auteur) de modèles, peut également créer des modèles. Les développeurs doivent continuer à configurer l’environnement, créer les bibliothèques clientes ainsi que les composants à utiliser, mais une fois ces éléments fondamentaux en place, le créateur de modèles a la possibilité de créer et de configurer des modèles sans projet de développement.
