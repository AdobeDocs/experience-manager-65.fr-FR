...
>[!NOTE]
>
>Pour plus d’informations sur les instructions de développement des gestionnaires de synchronisation de contenu, voir Gestionnaires d’application prêts à l’emploi, voir [Développement de gestionnaires de synchronisation de contenu](/help/mobile/contentsync-app-handlers.md).
...
* **includeFolders** - Le type est un tableau de chaînes qui permet de spécifier des dossiers supplémentaires à analyser dans la bibliothèque cliente pour récupérer des fichiers (tels que des polices personnalisées).
