...
1. Dans le [!UICONTROL modifier les valeurs de configuration ;] , spécifiez le nombre maximal de formulaires ou documents, une instance de l’AEM [!DNL Forms] Le serveur peut mettre en cache dans la variable **[!UICONTROL Nombre de Forms adaptatives]** champ . La valeur par défaut est 100.
...
## (Facultatif) Configurez le cache de formulaire adaptatif sur Dispatcher {#configure-the-cache}
...
1. Ouvrez l’URL suivante pour chaque instance de publication de votre environnement et [Activation de l’agent de vidage pour les instances de publication de votre environnement](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/page-invalidate.html?lang=fr#invalidating-dispatcher-cache-from-a-publishing-instance):
...
### Certains formulaires adaptatifs contenant des images ou des vidéos ne sont pas automatiquement invalidés du cache de Dispatcher. {#videos-or-images-not-auto-invalidated}
