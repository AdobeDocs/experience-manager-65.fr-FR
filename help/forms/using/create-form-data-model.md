...
description: Découvrez comment configurer MySQL comme source de données, créer un modèle de données de formulaire (FDM), le configurer et le tester pour AEM Forms.
...
1. Installez le pilote JDBC pour la base de données [!DNL MySQL] en tant que bundle OSGi :
   ...
   1. Sélectionnez **[!UICONTROL Installer/Mettre à jour]**. Une boîte de dialogue [!UICONTROL Télécharger/installer les bundles] s’affiche.

   1. Sélectionner **[!UICONTROL Choisir un fichier]** pour parcourir et sélectionner le [!DNL MySQL] Groupe OSGi du pilote JDBC. Sélectionner **[!UICONTROL Lot de démarrage]** et **[!UICONTROL Actualisation des modules]**, puis sélectionnez **[!UICONTROL Installation ou mise à jour]**. Assurez-vous que le pilote JDBC [!DNL Oracle Corporation's] pour [!DNL MySQL] est actif. Le pilote est installé.
...
1. Configurez la base de données [!DNL MySQL] comme source de données :
   ...
   1. Dans la boîte de dialogue de configuration, indiquez ce qui suit :
      ...
      * **Classe** du pilote JDBC : spécifiez le nom de classe Java™ du pilote JDBC. Pour la base de données [!DNL MySQL], spécifiez **com.mysql.jdbc.Driver**.
      * **URI de connexion JDBC** : spécifiez l’URL de connexion de la base de données. Pour [!DNL MySQL] base de données s’exécutant sur le port 3306 et le schéma `weretail`, l’URL est : `jdbc:mysql://'server':3306/weretail?autoReconnect=true&useUnicode=true&characterEncoding=utf-8`
...
1. Sélectionner **[!UICONTROL Créer]** > **[!UICONTROL Modèle de données de formulaire]**.
...
1. Développez l’arborescence de la source de données WeRailMySQL. Sélectionnez les objets et services de modèle de données suivants dans **weretail** > **customerdetails** schéma afin de pouvoir former un modèle de données :
...
1. Configurez les services de lecture et d’écriture pour le modèle de données de formulaire.
   ...
   1. Sélectionnez l’objet de modèle de **données customerdetails** , puis choisissez **[!UICONTROL Modifier les propriétés]**.
   1. Sélectionnez **[!UICONTROL get]** dans la liste déroulante Service de lecture. L’argument **id** qui est la clé principale de l’objet de modèle de données customerdetails est ajouté automatiquement. Sélectionner ![aem_6_3_edit](assets/aem_6_3_edit.png) et configurez l’argument comme suit.
...
1. Accédez à l’onglet **[!UICONTROL Services]** et configurez les services **[!UICONTROL get]** et **[!UICONTROL update]**.
   ...
   1. Sélectionnez la variable **[!UICONTROL get]** service et sélectionnez **[!UICONTROL Modifier les propriétés]**. La boîte de dialogue Propriétés s’ouvre.
   1. Spécifiez les éléments suivants dans la boîte de dialogue Modifier les propriétés :
      ...
      * **Description** : spécifiez la description contenant le fonctionnement détaillé du service. Par exemple :
        ...
        Ce service récupère l’adresse de livraison et les autres détails du client à partir de la variable [!DNL MySQL] base
      ...
      * **Arguments** : sélectionnez l’argument nommé **ID**.
      ...
      Sélectionner **[!UICONTROL Terminé]**. Le service de récupération des détails des clients de la base de données MySQL est configuré.

      ![shiiping-address-retrieval](assets/shiiping-address-retrieval.png)

   1. Sélectionnez la variable **[!UICONTROL update]** service et sélectionnez **[!UICONTROL Modifier les propriétés]**. La boîte de dialogue Propriétés s’ouvre.
   ...
   1. Spécifiez les éléments suivants dans la boîte de dialogue [!UICONTROL Modifier les propriétés] :
      ...
      * **Arguments**: sélectionnez le nom de l’argument **ID** et **customerdetails**.

      Sélectionner **[!UICONTROL Terminé]**. Le service **[!UICONTROL update]** permettant de mettre à jour les détails du client dans la base de données est configuré.[!DNL MySQL]
