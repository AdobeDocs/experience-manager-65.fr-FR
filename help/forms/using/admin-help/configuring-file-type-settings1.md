...
Le paramètre Exporter le type de fichier dans la section des paramètres d’exportation Adobe PDF permet de convertir un fichier PDF dans un autre format. Le format par défaut est HTML 4.01 avec des feuilles de style en cascade (CSS) 1.0(&amp;ast;.htm, &amp;ast;.html).
...
**[!UICONTROL Activer l’accessibilité et la conversion de contenu avec un fichier Adobe PDF balisé]** : permet d’incorporer les balises dans le fichier PDF. Cette option est désélectionnée par défaut.
