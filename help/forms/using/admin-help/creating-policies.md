...
**Copier :** Permet à l’utilisateur de copier du texte d’un protégé par cette stratégie.
...
**Hors ligne :** Permet à l’utilisateur de  un  hors ligne protégé par cette stratégie.

**Validité des autorisations :** Sélectionnez Permissions Are Always Valid ou définissez une période de validité des  d’autorisations du. Si vous sélectionnez une période de validité, cliquez sur les icônes de calendrier pour sélectionner une date et utilisez les flèches pour spécifier l’heure au format 24 heures. 
...
**Révoquer :** autorise l’éditeur  à révoquer les privilèges d’accès .

**Changer :** Permet à l’éditeur  de changer de privilège de stratégie.

### Paramètres généraux {#general-settings}

La zone Paramètres généraux contient les paramètres suivants :

**Période de validité :** Période pendant laquelle le  protégé par une stratégie est accessible aux autorisés. Plusieurs périodes de validité sont proposées :

**de ne sera pas valide après :** Le  de est accessible pendant le nombre de jours spécifié à partir du moment où le a été sécurisé.

**ne sera pas valide après cette date :** Le  de est valide à partir de la date à laquelle la stratégie est appliquée au  jusqu’à la date de fin spécifiée.

**Valide de de, à :** Le  est valide pendant les dates que vous avez spécifiées. Le cas échéant, vous pouvez utiliser le calendrier pour sélectionner une date. Pour ce faire, cliquez sur l’icône de calendrier.

**de est toujours valide :** La période de validité  du n’expire pas.
...
**Période de location hors ligne automatique :** nombre maximal de jours pendant lesquels le peut utiliser le protégé par une stratégie hors ligne (sans connexion Internet ou réseau active). A l’issue de cette période d’ouverture, le destinataire doit resynchroniser le document pour continuer à l’utiliser.
...
**Filigrane dynamique :** Sélectionnez un filigrane à afficher dynamiquement sur les pages d’un  (par exemple, lorsqu’un imprime le ). Les filigranes dynamiques identifient un document de manière unique, garantissant ainsi sa confidentialité et empêchant toute violation du copyright. Par exemple, l’administrateur peut configurer un filigrane dynamique qui affiche la date, le nom de l’utilisateur ou l’ID de la personne utilisant le document, ou encore le nom de la stratégie utilisée pour protéger le document. Un filigrane permet également d’afficher du texte personnalisé ou des éléments graphiques si la configuration le permettant a été effectuée. Les administrateurs configurent les options des filigranes et, tout comme les utilisateurs, peuvent les appliquer à des stratégies 
...
**Message d’erreur d’accès refusé :** Message qui s’affiche pour quiconque tente d’ouvrir un protégé par une stratégie sans autorisation. Ce message apparaît dans Acrobat. Les clients qui ne peuvent pas afficher ce message affichent un message par défaut pour indiquer que l’accès est refusé.
...
**Algorithme de chiffrement et longueur de clé :** Utilisé pour protéger votre . Faites votre choix parmi les options suivantes :
...
**Restrictions  :** Sélectionnez les composants  PDF à chiffrer. D’autres applications clientes chiffrent le document entier, mais pas les fichiers liés ou incorporés. Faites votre choix parmi les options suivantes :
