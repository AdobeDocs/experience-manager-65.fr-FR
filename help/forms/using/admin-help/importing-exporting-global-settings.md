...
**server_remoteevents_ClientTimeoutMilliseconds :** durée maximale pendant laquelle le JGroup attend les messages de . Ce paramètre doit rester inchangé.
...
**server_remoteevents_JChannelConnectionProperties:** Propriétés de connexion de JGroup utilisées pour communiquer entre le serveur (sur lequel un de service est traité par le service RemoteEvent) et toutes les instances de Workspace.
...
**server_remoteevents_JGroupName :** Nom du JGroup utilisé pour la communication de l’à distance. Pour éviter les conflits dans les grappes, cette valeur est générée de manière aléatoire. Cette valeur doit rester inchangée.
...
**client_routes_formViewOnly :** Lorsqu’elle est définie sur True, les itinéraires ne s’affichent pas dans les  de carte ou les  de  dans Workspace. La valeur par défaut est False, ce qui signifie que les itinéraires sont affichés sous forme de carte ou de liste.

### Autres paramètres {#other-settings}

**client_mimeTypes_openOutsideBrowser :** Type MIME de  qui s’ouvrira en dehors de l’instance du navigateur de Workspace. Si les processus de votre entreprise nécessitent un type MIME supplémentaire, indiquez-le ici. Les valeurs par défaut sont :

* `application/msword`
* `application/msexcel`
* `application/ms-powerpoint`

**client_customUI_caching :** Met en cache une interface utilisateur de  personnalisée.

**server_debugLevel :** Ne modifiez pas ce paramètre.

**client_pollingInterval :** Définit l’intervalle d’interrogation (en secondes) utilisé sur l’espace de travail Flex (obsolète pour AEM forms on JEE) pour détecter les  de nouvelles et modifiées. La valeur par défaut est de 3 secondes. Elle ne fonctionne pas pour l’espace de travail AEM Forms

**client_systemContext_name :** Spécifiez un nom personnalisé (par exemple, Citoyen) à afficher dans le champ Ajouté par (dans l’onglet Pièces jointes) pour les pièces jointes d’un  dans l’espace de travail AEM Forms.
...
>[!NOTE]
>
>pour l’application de démonstration, la valeur par défaut du nom d’affichage est **Citoyen**. Pour une application personnalisée que vous avez créée, le nom d’affichage par défaut est **Compte de contexte du système**.***client_idleTimeout :** Lorsqu’un utilisateur reste inactif pendant une durée spécifique, la session de l’espace de travail AEM Forms expire. Pour activer la fonctionnalité, ajoutez une entrée aux Paramètres généraux &lt;client_idleTimeout>*IDLE_TIMEOUT_IN_SECONDS*&lt;/client_idleTimeout>. Vous pouvez spécifier la valeur 0 pour désactiver le délai d’inactivité. La durée est spécifiée en secondes.
