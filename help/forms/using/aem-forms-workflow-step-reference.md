...
* **** Utiliser la communication interactive : Spécifiez la méthode pour localiser la communication interactive d’entrée. Vous pouvez utiliser la communication interactive envoyée au flux de travail, disponible à un chemin absolu ou disponible à un chemin dans une variable. Vous pouvez utiliser une variable de type String pour spécifier le chemin d’accès. Cette option est disponible si vous sélectionnez Interface utilisateur de l’agent de communication interactive ou Document de canal Web de communication interactive dans la liste déroulante Type.\
   **** Remarque : Pour accéder à l’interface utilisateur de l’agent de communications interactives dans la boîte de réception AEM, vous devez disposer des attributions de groupe cm-agent-users et workflow-users.
...
* **** Prérempli : Les champs suivants répertoriés ci-dessous servent d’entrées à la tâche :
   ...
   * **** Sélectionnez les pièces jointes d’entrée à l’aide de : Les pièces jointes disponibles à l’emplacement sont jointes au formulaire associé à la tâche. Le chemin d’accès est toujours relatif à la charge. An example path is [Payload_Directory]/attachments/
   ...
   * **** Sélectionnez un service de préremplissage personnalisé : Sélectionnez le service de préremplissage pour récupérer les données et préremplissez le document du canal Web Interactive Communication ou l’interface utilisateur de l’agent.
   ...
   * **** Request Attribute Mapping : Utilisez la section Mappage d’attributs de requête pour définir le [nom et la valeur de l’attribut](../../forms/using/work-with-form-data-model.md#bindargument)de requête. Récupérez les détails de la source de données en fonction du nom d’attribut et de la valeur spécifiés dans la requête. Vous pouvez définir une valeur d’attribut de requête à l’aide d’une valeur littérale ou d’une variable de type de données String.\
      Les options de mappage du service de préremplissage et des attributs de requête ne sont disponibles que si vous sélectionnez Interface utilisateur de l’agent de communication interactive ou Document de canal Web de communication interactive dans la liste déroulante Type.
...
* **Utilisateur ou groupe :** la tâche est affectée à l’utilisateur ou au groupe sélectionné. Cette option est disponible lorsque l’option **À un utilisateur ou un groupe spécifique** est activée dans le champ Options d’affectation. **** Le champ répertorie tous les utilisateurs et groupes du groupe d’utilisateurs du processus.\
   Le menu déroulant **Utilisateur ou Groupe** répertorie les utilisateurs et groupes auxquels l’utilisateur connecté a accès. L’affichage du nom d’utilisateur dépend de si vous disposez d’autorisations d’accès sur le noeud **users** dans crx-repository pour cet utilisateur particulier.
...
* **** Paramètres de partage : La boîte de réception AEM propose des options pour partager une seule tâche ou toutes les tâches de la boîte de réception avec un autre utilisateur :
...
* **Enregistrez les pièces jointes de la tâche de sortie à l’aide** de : Spécifiez l’emplacement du dossier des pièces jointes. Vous pouvez enregistrer des pièces jointes de tâche de sortie à l’aide d’un chemin d’accès relatif à la charge utile ou d’une variable de tableau de type de données de document. Cette option s’affiche uniquement si vous cochez la case **Autoriser les personnes désignées à ajouter des pièces jointes à la tâche** et cochez la case Formulaire **** adaptatif, Formulaire **adaptatif en** lecture seule ou Document PDF **non interactif dans la liste déroulante Type de l’onglet Formulaire/Document.**********

>[!NOTE]
>
>Utilisez l’onglet Pièces jointes de l’interface utilisateur de l’agent au moment de l’exécution pour associer les pièces jointes à une communication interactive. Les pièces jointes associées s’affichent sous forme de pièces jointes de tâche dans le panneau latéral après l’ouverture de la tâche dans un état Terminé.
...
* **Input for services > Fournir des données d’entrée à l’aide de métadonnées littérales, variables ou de flux de travail, et d’un fichier** JSON : Un service peut comporter plusieurs arguments. Sélectionnez l’option pour obtenir la valeur des arguments de service à partir d’une propriété de métadonnées de flux de travail, d’un objet JSON, d’une variable ou saisissez directement la valeur dans la zone de texte fournie :
   ...
   * **Mapper les champs de saisie depuis le fichier JSON d’entrée :** spécifiez le chemin d’accès d’un fichier JSON pour obtenir la valeur d’entrée des arguments de service à partir du fichier JSON. Le chemin d’accès du fichier JSON peut être relatif à la charge utile, un chemin absolu ou vous pouvez sélectionner un document JSON d’entrée à l’aide d’une variable JSON ou d’un type de modèle de données de formulaire.

* **** Input for services > Fournir des données d’entrée à l’aide d’une variable ou d’un fichier JSON : Sélectionnez l’option pour obtenir des valeurs pour tous les arguments d’un fichier JSON enregistré à un chemin absolu, à un chemin relatif à la charge utile ou dans une variable.
...
* **Paramètre régional :** spécifiez la langue pour les options de messagerie et de vérification. Vous pouvez stocker le paramètre régional dans une variable de type de données String ou sélectionner **Literal** pour choisir le paramètre régional dans la liste des options disponibles. Vous devez définir le code de paramètre régional lors du stockage de la valeur du paramètre régional dans une variable. Par exemple, spécifiez **en_US** pour l’anglais et **fr_FR** pour le français.
...
* **** Sélectionnez le document à signer à l’aide de : Vous pouvez choisir un document à un emplacement relatif à la charge, utiliser la charge utile comme document, spécifier un chemin absolu du document ou récupérer le document stocké dans une variable de type de données Document.
...
Tout comme les autres étapes du processus AEM Forms, telles que Affecter une tâche, Envoyer un courrier électronique et Signer un document, vous pouvez utiliser des variables dans toutes les étapes des services de document AEM. Pour plus d’informations sur la création et la gestion des variables, voir [Variables dans les processus](../../forms/using/variable-in-aem-workflows.md)AEM.

### Étape Appliquer l’horodatage du document {#apply-document-time-stamp-step}

Ajoute un horodatage à un document. Indiquez les détails du document, tels que le chemin d’accès du document d’entrée, le nom du document d’entrée, l’emplacement de stockage des données exportées. Vous pouvez choisir de remplacer le fichier de charge utile existant, choisir un nom de fichier différent pour stocker les données dans un fichier différent sous le dossier de charge, fournir un chemin d’accès absolu aux données ou stocker les données dans une variable de type Document.

### Etape Convertir en image {#convert-to-image-step}

permet de convertir un document PDF en liste d’images. Les formats d’image pris en charge sont JPEG, JPEG2000, PNG et TIFF. Les informations suivantes s’appliquent aux conversions en images TIFF :
...
Génère un document PDF à partir d’un fichier d’entrée. Le document d’entrée peut être relatif à la charge utile, avoir un chemin d’accès absolu, être la charge elle-même ou être stocké dans une variable du type de données Document.
