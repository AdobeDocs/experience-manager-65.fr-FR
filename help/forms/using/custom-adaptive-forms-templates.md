...
* Modèle Survey : Vous permet de créer un formulaire adaptatif d’une seule page à l’aide de la disposition réactive qui comporte plusieurs colonnes configurées. La disposition s’ajuste automatiquement selon les dimensions des différents écrans sur lesquels vous souhaitez afficher le formulaire.
...
* Modèle Tabbed Enrollment (modèle d&#39;inscription avec onglet) : Permet de créer un formulaire adaptatif à plusieurs onglets à l’aide d’une disposition d’onglets sur la gauche, dans laquelle vous pouvez consulter les onglets de manière aléatoire.
