...
seo-title: '"Tutoriel : Créer votre premier formulaire adaptatif"'
...
* Obtenez le pilote de base de données JDBC (fichier JAR) auprès du fournisseur de base de données. Les exemples du tutoriel sont basés sur la base de données [!DNL MySQL] et utilisent [!DNL Oracle's] [le pilote de base de données JDBC MySQL](https://dev.mysql.com/downloads/connector/j/5.1.html).
...
* Configurer l’instance de base de données du site web ([!DNL MySQL] base de données) comme source de données
* Créez le modèle de données de formulaire à l’aide de la base de données [!DNL MySQL] comme source de données.
...
Les formulaires adaptatifs font partie intégrante de vos interactions avec les clients. Il est important de tester vos formulaires adaptatifs à chaque modification que vous y apportez. Tester tous les champs d’un formulaire est fastidieux. AEM [!DNL Forms] fournir un SDK (Calvin SDK) pour automatiser le test des formulaires adaptatifs. Calvin vous permet d’automatiser les tests de vos formulaires adaptatifs dans le navigateur Web.
...
Vous pouvez publier des formulaires adaptatifs sous la forme d’un formulaire autonome (application d’une seule page), inclure dans AEM [page Sites](/help/forms/using/embed-adaptive-form-aem-sites.md) ou une liste sur une AEM [!DNL Site] à l’aide de [Portail Forms](../../forms/using/introduction-publishing-forms.md).

Objectifs:

* Publier le formulaire adaptatif en tant que page AEM
* Incorporer le formulaire adaptatif dans une page [!DNL Sites] AEM
