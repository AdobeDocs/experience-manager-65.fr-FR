...
1. Définissez les options d’exécution du rendu.
   ...
   * Mettez en cache la conception de formulaire afin d’améliorer les performances du service Output en appelant le formulaire `RenderOptionsSpec` et en transmettant `setCacheEnabled` `true`.
   >[!NOTE]
   >
   >Vous ne pouvez pas définir la version du document PDF à l’aide de la `RenderOptionsSpec` `setPdfVersion` méthode de l’objet si le document d’entrée est un formulaire Acrobat (formulaire créé dans Acrobat) ou un document XFA signé ou certifié. Le document PDF de sortie conserve la version PDF d’origine. De même, vous ne pouvez pas définir l’option Adobe PDF balisé en appelant la `RenderOptionsSpec` `setTaggedPDF` méthode de l’objet si le document d’entrée est un formulaire Acrobat ou un document XFA signé ou certifié.
...
1. Récupérez les résultats de l’opération.
   ...
   * Appelez la `com.adobe.idp.Document` méthode de l’ `copyToFile` objet pour copier le contenu de l’ `com.adobe.idp.Document` objet dans le fichier (veillez à utiliser l’ `com.adobe.idp.Document` objet renvoyé par la `getStatusDoc` méthode).
   >[!NOTE]
   >
   >Bien que le service Output écrive le document PDF/A à l’emplacement spécifié par l’argument transmis à la `PDFOutputOptionsSpec` méthode de l’ `setFileURI` objet, vous pouvez récupérer le document PDF/A par programmation en appelant la `OutputResult` `getGeneratedDoc` méthode de l’objet.
