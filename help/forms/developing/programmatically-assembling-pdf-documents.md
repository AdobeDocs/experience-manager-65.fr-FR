...
1. Extrayez les résultats.
   ...
   * Effectuez une itération dans l’objet `java.util.Map` jusqu’à ce que vous trouviez l’objet `com.adobe.idp.Document` généré. (Vous pouvez utiliser l’élément de résultat PDF spécifié dans le document DDX pour obtenir le document).
...
1. Référencez les documents PDF d’entrée.
   ...
   * Renseignez le tableau d’octets avec les données de flux en appelant la méthode `Read` de lʼobjet `System.IO.FileStream`. Transmettez le tableau d’octets, la position de départ et la longueur du flux à lire.
