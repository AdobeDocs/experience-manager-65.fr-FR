...
Ce glossaire répertorie (par ordre alphabétique) les détails de tous les documents livrables du [Liste de contrôle du projet](/help/managing/best-practices-checklist.md).
...
Plus tôt vous planifiez et concevez vos tests d’acceptation, plus le déploiement final est facile. Elles doivent être définies avec le client et votre équipe d’assurance qualité.
...
Assurez-vous que les personnes appropriées sont enregistrées pour prendre en compte les [examens de certification](https://training.adobe.com/certification/exams.html#p=1&amp;solution=adobeExperienceManager).
...
Les indicateurs de performance clés (IPC) aident une organisation à définir et à mesurer les progrès vers les objectifs de l’organisation. Une fois qu&#39;une organisation a analysé sa mission et défini ses objectifs, elle doit mesurer les progrès vers ces objectifs. Les indicateurs de performance clés constituent un mécanisme de mesure.
...
La feuille de route client est composée de jalons de haut niveau et d’objectifs commerciaux. La chronologie du projet doit respecter et s’aligner sur cette stratégie, de sorte que tous les risques potentiels et/ou les écarts possibles doivent être mis en évidence et suivis.
...
Assurez-vous que votre équipe est composée du personnel disposant de la formation appropriée. Pour les équipes de projet, il est recommandé d’avoir tous les éléments suivants :
...
Vous obtenez ainsi une vue de haut niveau du système et de l’architecture de la solution. À ce stade, il s’agit d’un projet qui sera révisé et affiné ultérieurement.
...
Le comité de révision doit être représentatif de toutes les parties prenantes clés impliquées dans l’architecture. En règle générale, ils sont composés d’un groupe de cadres responsables de la révision et de la maintenance de l’architecture globale.
...
Le concept de sauvegarde et de restauration décrit les fonctionnalités techniques qui seront mises en oeuvre dans la solution. Il est nécessaire pour la politique de sauvegarde et de restauration de l’entreprise.
...
### Analyses de cas {#business-case-s}
...
La composition des modèles et des composants utilisés dans la nouvelle application. Inclut des détails tels que les règles d’héritage, les autorisations et les relations, entre autres.
...
Il s’agit souvent de calendriers, de planification et d’exigences de validation.
...
Toute stratégie, exigence ou les deux, du client en ce qui concerne la création de rapports. Il peut s’agir :
...
Stratégies formalisées couvrant le déploiement et les versions de votre projet. Il peut s’agir :
...
Un chemin heureux est un scénario par défaut ne présentant aucune condition exceptionnelle ou d’erreur. Il est composé de la séquence des activités exécutées lorsque tout se passe comme prévu.
...
La conception de solution de haut niveau explique l’architecture utilisée pour développer la solution. Le diagramme d’architecture fournit un aperçu de l’ensemble du système, identifiant les principaux composants développés pour le produit et leurs interfaces.
...
Migration ainsi que la validation du contenu et d’autres artefacts pour la nouvelle solution.
...
Le concept de mise en œuvre fournit les principes directeurs de l’ensemble de la mise en œuvre. Il doit tenir compte des points suivants :
...
Ce concept décrit également les structures, bibliothèques et autres artefacts utilisés dans la solution.
...
Tous les intervenants du projet doivent avoir accès à pour faciliter la transparence de l’état du projet.
...
* atteindre les indicateurs clés de performance
...
Document conceptuel sur la manière de vous assurer que votre mise en oeuvre respecte les indicateurs de performance clés et de mettre à l’échelle la solution afin qu’elle continue à satisfaire ces indicateurs de performance clés.
...
Les tests basés sur le personnage sont une méthode basée sur les différentes personnes décrites dans les conceptions d’expérience. Il teste également les comptes et leurs niveaux d’autorisations associés.
...
Il doit viser à démontrer la faisabilité de la solution, à vérifier qu’elle peut remplir l’objectif requis et à prouver qu’elle est potentiellement utilisée.
...
L’évaluation des risques est effectuée par le service informatique du client, le service de sécurité, ou les deux.
...
* d’utilisateurs ;
* autorisations ;
* et gestion des utilisateurs et attribution des privilèges d’accès
...
Définissez et documentez les détails de la configuration de sécurité requise pour l’application, l’architecture et l’infrastructure.

### Version préliminaire du concept de sécurité {#security-concept-draft}

Présentation de haut niveau couvrant la configuration de sécurité de :
...
Le conseil d’examen des solutions est composé des parties prenantes des clients.
...
Le processus d’approbation et d’acceptation définit les critères qui doivent être remplis avant que la solution puisse être publiée dans un environnement de production.
...
Un processus clair pour l’approbation des spécifications par le client doit être mis en place. Ce processus garantit la clarté et la fermeté de la portée des exigences.
...
Tous les facteurs de risque figurant dans l’évaluation des risques (ou dans d’autres révisions) sont identifiés et évalués :
...
Matériel à utiliser dans les sessions de formation. Les documents doivent être créés spécifiquement pour la solution et conçus pour être utilisés avec les guides de l’utilisateur.
...
Sur la base du concept de processus, ces spécifications doivent définir, en détail, les étapes de création du processus complet.
