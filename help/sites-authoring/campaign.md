...
1. Sélectionnez l’un des modèles disponibles auxquels vous vous connectez, puis cliquez sur **Suivant**. Trois modèles sont disponibles par défaut :
...
Vous pouvez sélectionner le service cloud à partir de la **Sites** ou dans l’email/la newsletter après sa création.

Sélection du service cloud à partir de la variable **Sites** est l’approche recommandée. La sélection du service cloud à partir du courrier électronique/de la newsletter nécessite une solution.
...
La manière dont vous liez ou synchronisez AEM avec Adobe Campaign dépend de l’utilisation d’un Adobe Campaign Standard par abonnement ou d’un Adobe Campaign Classic sur site.
...
1. Sélectionnez le contenu que vous souhaitez synchroniser avec votre diffusion.
   ...
   * Libellé du contenu dans AEM.
   * État d’approbation du contenu dans AEM. Si le contenu n&#39;est pas validé, vous pouvez le synchroniser, mais il devra être validé avant l&#39;envoi de la diffusion. Cependant, vous pouvez exécuter certaines opérations, telles que l’envoi d’un test BAT ou le test d’aperçu.
   * Date de la dernière modification du contenu.
   * Tout contenu déjà lié à une diffusion.
