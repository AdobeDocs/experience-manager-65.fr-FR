...
Les composants suivants sont destinés à être utilisés lors de la création de contenu pour une page web standard. Les composants forment un sous-ensemble des composants prêts à l’emploi pour une installation standard d’AEM.
...
Il existe plusieurs façons de créer une liste de pages : Pages enfants, Liste fixe, Recherche ou Recherche avancée (toutes décrites ci-dessous). Quelle que soit la méthode choisie, les pages que vous incluez dans votre liste doivent déjà être associées à une image. C’est cette image qui s’affiche dans le carrousel. S’il n’existe aucune image pour une page sous les Propriétés de page de cette page, vous devez associer une image à la page avant de commencer. Sinon, le carrousel affiche une page vierge. Voir [Modification des propriétés de page](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md).
...
>[!NOTE]
>
>Vous pouvez créer un composant de carrousel personnalisé pour Adobe Experience Manager qui affiche des ressources numériques dans la gestion des actifs numériques AEM.
...
Effectuez un choix parmi les types suivants : Histogramme, Graphique en secteurs et Graphique en courbe.
   ...
   * **Texte de remplacement**
Texte qui s’affiche à la place du graphique.
...
Permet de charger un fichier zip contenant un module de conception.
...
Courte description affichée avec le lien de téléchargement.
   ...
   * **Fichier**
Le fichier qui peut être téléchargé sur la page Web qui en résulte. Faites glisser une ressource à partir de l’outil de recherche de contenu ou sélectionnez la zone de sorte que vous puissiez télécharger le fichier que vous souhaitez rendre disponible pour téléchargement.
...
L’exemple suivant illustre une **Liste** de la manière dont il peut afficher une liste de pages enfants (la conception est contrôlée par les définitions CSS personnalisées d’une conception de site).
...
Pour spécifier le chemin d’accès à un paragraphe, vous devez ajouter le suffixe suivant (à la page) :
...
Outre le référencement d’un paragraphe spécifique, le chemin peut également être modifié pour spécifier un système de paragraphes entier. Combinez le chemin d’accès avec ce qui suit :
...
* **Diapositives**
   ...
   * **Titre**
      ...
      Si nécessaire, indiquez un titre. Ce titre est superposé sur la diapositive appropriée.
...
Le composant Diaporama affiche ensuite de manière répétée chacune d’elles de manière séquentielle, pendant une courte période, avant de se fondre dans la diapositive suivante :
...
>
>Adobe recommande d’utiliser la variable **Tableau** pour les tableaux, bien qu’ils puissent également être construits avec la propriété **Texte** composant.

Le **Tableau** est préconfiguré pour vous permettre de créer, de remplir et de formater un tableau. La boîte de dialogue vous permet de configurer votre tableau et de créer le contenu à partir de zéro, ou en copiant et en collant une feuille de calcul ou un tableau à partir d’un éditeur externe, tel qu’Excel, OpenOffice ou Notepad.
...
Lorsque vous ouvrez le **Styles** de l’onglet **Modifier** vous pouvez également définir les éléments suivants :
...
* **Propriétés de l’image** (**Propriétés d’image avancées**)
   ...
   * **Titre**
      ...
      Titre du bloc. Elle est affichée par le pointeur de la souris.
...
* Vidéo
   ...
   * **Taille**
      ...
      La taille native de la vidéo (largeur x hauteur en pixels) s’affiche dans les zones en regard de Taille (voir ci-dessus). Saisissez ici manuellement la largeur et la hauteur pour remplacer les dimensions natives de la vidéo. Sélection **OK** ferme la boîte de dialogue.
...
L’exemple suivant illustre les composants Colonnes et Colonnes utilisés. Vous pouvez utiliser des espaces réservés pour les nouveaux composants :
...
* **Contraintes**
   ...
   Vous pouvez indiquer ici si un champ est obligatoire et y placer des contraintes, telles que la valeur numérique.
...
Ce champ est obligatoire et ne doit contenir que les caractères suivants :
   ...
   * **Description**
      ...
      Vous permet de fournir des informations supplémentaires à l’utilisateur, si nécessaire. Sur le formulaire, cette description s’affiche sous le champ, dans une police plus petite que le titre.
...
* **Contraintes**
   ...
   * **Requis**
      ...
      Cette contrainte dépend du type de composant de formulaire, mais fournit une ou plusieurs zones de clic pour indiquer que ce champ est obligatoire ou que certaines parties de ce champ sont obligatoires.

   * **Message obligatoire**

      Message informant les utilisateurs que ce champ est obligatoire. Un champ obligatoire est marqué d’un astérisque.
...
Le composant Formulaire définit le début et la fin d’un formulaire à l’aide de la fonction **Début du formulaire** et **Fin de formulaire** éléments . Les éléments de début et de fin sont toujours associés pour s’assurer que le formulaire est correctement défini.
...
Le champ **Chemin de chargement** vous permet de précharger des valeurs dans les champs obligatoires du formulaire. Consultez [Préchargement des valeurs de formulaire](/help/sites-developing/developing-forms.md#preloading-form-values).
   ...
   * **Validation du client**
      ...
      Indique si la validation du client est requise pour ce formulaire (la validation du serveur est *toujours* effectuée). La validation est effectuée avec la fonction **Captcha Forms** composant.
...
Spécifiez la liste de prospects requise.
      * **Créer un compte et le mettre à jour**
         ...
         * **Afficher des données...**
            ...
            Cliquez sur le bouton pour accéder aux informations sur les résultats du formulaire dans l’éditeur en bloc. Vous avez alors la possibilité d’exporter les informations vers un fichier `.tsv` (séparé par des tabulations) (en vue de l’utiliser, par exemple, dans une feuille de calcul Excel).
      * **Courrier**
...
Ce composant permet de créer un champ masqué. Ces champs masqués peuvent être utilisés à diverses fins, par exemple lorsque vous devez effectuer une action après l’envoi du formulaire ou lorsque des données masquées sont requises lors du post-traitement.
...
Un bouton d&#39;image permet de créer un bouton avec votre propre image et texte :
...
Permet à un utilisateur de saisir son mot de passe :
...
Vous pouvez spécifier le nom de l’élément ainsi qu’un titre et une description. A l’aide des boutons + et -, vous pouvez ajouter ou supprimer des éléments, les positionner avec les flèches haut et bas et spécifier une valeur par défaut, le cas échéant :
...
Vous pouvez spécifier différents paramètres, y compris les espaces de noms qui peuvent être utilisés, à l’aide de l’onglet spécialisé :
...
### Boutons d’envoi de workflow {#workflow-submit-button-s}
