...
>[!NOTE]
>
>Si les ressources Dynamic Media Classic (Scene7) ne s’affichent pas correctement, assurez-vous que Dynamic Media est [désactivé](/help/assets/config-dynamic.md#disabling-dynamic-media), puis actualisez la page.
...
>[!NOTE]
>
>Experience Manager publie sur Dynamic Media Classic (Scene7) de manière asynchrone. Une fois que vous avez sélectionné **[!UICONTROL Publier]**, la publication de votre ressource vers Dynamic Media Classic (Scene7) peut prendre plusieurs secondes.
...
1. Dans l’interface utilisateur classique de Experience Manager, sélectionnez **[!UICONTROL Ressources numériques]** pour accéder au gestionnaire de ressources numériques.

1. Sélectionnez la ou les ressources ou le dossier dans le dossier cible que vous souhaitez publier dans Dynamic Media Classic (Scene7), cliquez avec le bouton droit de la souris et sélectionnez **[!UICONTROL Publier vers Dynamic Media Classic (Scene7)]**. Vous pouvez également sélectionner **[!UICONTROL Publier vers Dynamic Media Classic (Scene7)]** dans le menu **[!UICONTROL Outils]**.
...
1. Accédez à Dynamic Media Classic (Scene7) et vérifiez que les ressources sont disponibles.
   ...
   >[!NOTE]
   >
   >Si les ressources ne se trouvent pas dans un dossier synchronisé Dynamic Media Classic (Scene7), la fonction **[!UICONTROL Publier vers Dynamic Media Classic (Scene7)]** dans les deux menus est visible, mais désactivée.
...
>[!NOTE]
>
>Si la ressource ne se trouve pas dans le dossier synchronisé de Dynamic Media Classic (Scene7), le lien vers **[!UICONTROL Publier vers Dynamic Media Classic (Scene7)]** n’apparaît pas.

Pour publier vers Dynamic Media Classic (Scene7) directement à partir d’une ressource numérique :
...
1. Dans Experience Manager, dans l’IU classique, sélectionnez **[!UICONTROL Sites web]** et accédez à la page web à laquelle vous souhaitez ajouter une ressource numérique qui n’est pas encore publiée dans Dynamic Media Classic (Scene7). (Les règles normales d’héritage de la page s’appliquent.)

1. Dans le sidekick, sélectionnez l’icône **[!UICONTROL Page]** et sélectionnez **[!UICONTROL Propriétés de la page]**.

1. Sélectionnez **[!UICONTROL Cloud Services]**.
1. Sélectionnez **[!UICONTROL Ajouter des services]**.
1. Sélectionnez **[!UICONTROL Dynamic Media Classic (Scene7)]**.
1. Dans la liste déroulante **[!UICONTROL Adobe Dynamic Media Classic (Scene7)]** , sélectionnez la configuration souhaitée et cliquez sur **[!UICONTROL OK]**.

   ![chlimage_1-49](assets/chlimage_1-49.png)

1. Sur la page web, ajoutez un composant Dynamic Media Classic (Scene7) à l’emplacement souhaité sur la page.
1. Depuis l’outil de recherche de contenu, faites glisser un élément numérique vers le composant. Un lien s’affiche vers **[!UICONTROL Vérification de l’état de publication de Dynamic Media Classic (Scene7)]**.

   >[!NOTE]
   >
   >Si la ressource numérique se trouve dans le dossier cible CQ, aucun lien vers **[!UICONTROL Vérifier l’état de publication de Dynamic Media Classic (Scene7)]** ne s’affiche. Les ressources sont placées dans le composant.

   ![chlimage_1-50](assets/chlimage_1-50.png)

1. Sélectionnez **[!UICONTROL Vérification de l’état de publication de Dynamic Media Classic (Scene7)]**. Si les ressources ne sont pas publiées, Experience Manager les publie dans Dynamic Media Classic (Scene7). Une fois la ressource chargée, elle se trouve dans le dossier à la demande. Par défaut, le dossier à la demande se trouve dans le **[!UICONTROL nom_de_la_société/CQ5_adhoc]**. Vous pouvez [configurer le dossier à la demande, le cas échéant](#configuringtheadhocfolder).

   >[!NOTE]
   >
   >Si la ressource ne se trouve pas dans un dossier synchronisé Dynamic Media Classic (Scene7) et qu’aucune configuration de cloud Dynamic Media Classic (Scene7) n’est associée à la page active, le chargement échoue.
...
>[!NOTE]
>
>Si vous créez et développez des visionneuses S7 personnalisées et que vous utilisez l’outil de recherche de contenu, vous devez ajouter explicitement le paramètre `allowfullscreen` .
...
L’ajout d’un composant Dynamic Media Classic (Scene7) à une page est identique à l’ajout d’un composant à n’importe quelle page. Les composants de Dynamic Media Classic (Scene7) sont décrits en détail dans les sections suivantes.
...
1. Revenez au mode **Modifier** en sélectionnant le crayon dans le sidekick.

1. Faites glisser un composant du groupe **[!UICONTROL Dynamic Media Classic (Scene7)]** du sidekick vers la page à l’emplacement souhaité.
...
1. Connectez-vous à Experience Manager et assurez-vous que vous disposez des [Cloud Services Adobe Dynamic Media Classic (Scene7) ](/help/sites-administering/scene7.md#configuring-scene-integration) et que les composants Dynamic Media Classic (Scene7) sont disponibles.

   >[!NOTE]
   >
   >Si les composants de la gestion de contenu web de Dynamic Media Classic (Scene7) ne sont pas disponibles, veillez à les activer via le mode de conception.

1. Sur un site web dont les composants Dynamic Media Classic (Scene7) sont activés, faites glisser une visionneuse **[!UICONTROL Image]** vers la page.
...
* **Référence du fichier** : accédez à un fichier que vous souhaitez référencer. La référence de fichier affiche l’URL de la ressource et pas nécessairement l’URL complète de Dynamic Media Classic (Scene7), y compris les commandes d’URL et les paramètres. Vous ne pouvez pas ajouter de commandes d’URL et de paramètres Dynamic Media Classic (Scene7) dans ce champ. Au lieu de cela, ils doivent être ajoutés par le biais de la fonctionnalité correspondante dans le composant.
* **Largeur** : permet de définir la largeur.
* **Hauteur** : permet de définir la hauteur.

Vous définissez ces options de configuration en ouvrant (en double-cliquant) un composant Dynamic Media Classic (Scene7), par exemple, lorsque vous ouvrez un composant **Zoom** :
...
L’élément comporte des outils de zoom dans sa partie inférieure. Sélectionnez **[!UICONTROL +]** pour l’agrandir. Sélectionnez **[!UICONTROL -]** pour réduire. La sélection de la **[!UICONTROL x]** ou de la flèche de réinitialisation du zoom rétablit la taille d’origine de l’image importée sous la forme . Sélectionnez les flèches diagonales pour faire en sorte qu’elles s’affichent en plein écran. Sélectionnez **[!UICONTROL Modifier]** afin de pouvoir configurer le composant. Avec ce composant, vous pouvez configurer les [paramètres communs à tous les composants Dynamic Media Classic (Scene7)](#settings-common-to-all-scene-components).

![](do-not-localize/chlimage_1-3.png)

### Fenêtre déroulante {#flyout}

Dans le composant Fenêtre déroulante HTML5, l’élément s’affiche sous la forme d’un écran partagé : à gauche se trouve l’élément à la taille spécifiée, à droite la partie sur laquelle le zoom a été effectué. Sélectionnez **[!UICONTROL Modifier]** afin de pouvoir configurer le composant. Avec ce composant, vous pouvez configurer les [paramètres communs à tous les composants Dynamic Media Classic (Scene7)](/help/sites-administering/scene7.md#settingscommontoallscene7components).
...
Le composant d’image Dynamic Media Classic (Scene7) vous permet d’ajouter la fonctionnalité Dynamic Media Classic (Scene7) à vos images, par exemple les modificateurs Dynamic Media Classic (Scene7), les paramètres d’image ou de visionneuse prédéfinis et l’accentuation. Le composant d’image Dynamic Media Classic (Scene7) est similaire aux autres composants d’image dans Experience Manager avec des fonctionnalités spéciales Dynamic Media Classic (Scene7). Dans cet exemple, le modificateur d’URL Dynamic Media Classic (Scene7) est appliqué à l’image, `&op_invert=1`.
...
**Configuration de Dynamic Media Classic (Scene7)**  : sélectionnez la configuration de Dynamic Media Classic (Scene7) que vous souhaitez utiliser pour récupérer les principaux paramètres d’image prédéfinis à partir de SPS.
...
Sélectionnez **[!UICONTROL Modifier]** pour configurer le composant. Vous pouvez configurer les [paramètres communs à tous les composants de Dynamic Media Classic (Scene7)](/help/sites-administering/scene7.md#settingscommontoallscene7components) et aux autres paramètres décrits dans cette section.

![chlimage_1-55](assets/chlimage_1-55.png)

**Référence du fichier, largeur, hauteur**  : voir  [Paramètres communs à tous les composants](/help/sites-administering/scene7.md#settingscommontoallscene7components) Dynamic Media Classic (Scene7).
...
Pour modifier le texte de manière dynamique, saisissez le nouveau texte dans les champs et sélectionnez **[!UICONTROL OK]**. Dans cet exemple, le **Prix** est désormais de 50 $ et l’expédition de 99 cents.
...
1. Sur la page comportant le composant de modèle d’image Dynamic Media Classic (Scene7), sélectionnez **[!UICONTROL Modifier]**.
...
Si vous disposez de plusieurs configurations, Experience Manager affiche par défaut la [configuration par défaut](/help/sites-administering/scene7.md#configuring-a-default-configuration). Vous pouvez sélectionner différentes configurations directement dans l’explorateur de contenu Dynamic Media Classic (Scene7) du menu déroulant.
...
>* Lorsque la fonction [Aperçu sécurisé est activée](/help/sites-administering/scene7.md#configuring-the-state-published-unpublished-of-assets-pushed-to-scene), les ressources publiées et non publiées sur Dynamic Media Classic (Scene7) n’apparaissent pas dans le navigateur de contenu Dynamic Media Classic (Scene7).
>* Si vous ne voyez pas **[!UICONTROL Dynamic Media Classic (Scene7)]** ou l’icône **[!UICONTROL S7]** comme option dans le navigateur de contenu, vous devez [configurer Dynamic Media Classic (Scene7) pour qu’il fonctionne avec Experience Manager](/help/sites-administering/scene7.md).
...
Parcourez le contenu de Dynamic Media Classic (Scene7) en sélectionnant l’onglet **[!UICONTROL S7]** .
