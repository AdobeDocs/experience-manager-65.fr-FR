...
Ces consignes sont classées selon trois niveaux de conformité : Niveau A (le plus bas), niveau AA et niveau AAA (le plus élevé). Pour résumer, les niveaux sont définis comme suit :
...
* **Niveau AA :** Niveau d’accessibilité idéal à rechercher, dans lequel votre site atteint un niveau d’accessibilité amélioré, de sorte qu’il soit accessible à la plupart des personnes dans la plupart des situations utilisant la plupart des technologies. Pour atteindre ce niveau, tous les critères de réussite de niveau A et de niveau AA sont satisfaits.
...
>[!NOTE]
>
>Ce document utilise les éléments suivants :
>
...
Pour les images statiques, la règle de base consiste à fournir un équivalent textuel. Cette méthode est effectuée dans la variable **Texte de remplacement** field :

>[!NOTE]
>
>Certains composants prêts à l’emploi, tels que **Carrousel** et **Diaporama**, ne permettent pas d’ajouter des descriptions d’images sous forme de texte de remplacement. Lors de l’implémentation de versions de ces composants pour votre instance AEM, votre équipe de développement doit configurer ces composants pour prendre en charge `alt` attribut. Cela permet de s’assurer que les auteurs peuvent l’ajouter au contenu (voir [Ajout de la prise en charge d’éléments et d’attributs de HTML supplémentaires](/help/sites-administering/rte-accessible-content.md#add-support-for-more-html-elements-and-attributes)).
...
Par défaut, AEM ajoute un **texte de remplacement** à vos images. Pour l’IU classique, il existe deux scénarios différents pour la création de l’attribut par défaut, bien que la valeur par défaut ne soit pas suffisante en tant qu’alternative et doive probablement être modifiée dans la variable **Avancé** onglet des propriétés d’image :

* File :

Une image est téléchargée à partir du disque dur de l’utilisateur. Si vous ajoutez un composant d’image à une page, puis choisissez une image sur votre disque dur ou une autre source, la valeur par défaut pour le **Texte de remplacement** est `file`. Cette valeur doit être modifiée dans la variable **Avancé** propriétés de l’image. Cette valeur n’est pas affichée dans le champ **Texte de remplacement** mais lorsque la valeur est modifiée, la nouvelle valeur s’affiche dans le champ.

* Asset :

une image est ajoutée à partir du référentiel de ressources numériques. Si vous faites glisser une image du référentiel de ressources numériques vers une page web, la variable **Titre** et **Texte de remplacement** les valeurs de cette image sont extraites des métadonnées de cette image.
...
>[!NOTE]
>
>Si votre image est purement décorative (consultez [Création d’un texte secondaire adapté](#creating-good-text-alternatives)), vous pouvez saisir un espace dans le champ **Texte de remplacement** à l’aide de la barre d’espace. Cela crée un vide `alt` qui invite un lecteur d’écran à ignorer l’image.
...
* Si le contenu non textuel est un lien vers une autre page ou un autre document et qu’il n’existe aucun autre texte faisant partie d’un même lien, le texte secondaire de l’image doit indiquer la destination du lien. Il ne doit pas décrire l’image.
...
* Il est acceptable de spécifier un texte de remplacement vide (nul) pour une image, mais seulement si l’image ne comporte pas de texte de remplacement. Par exemple, il s’agit d’un graphique purement décoratif. Ou, si le texte équivalent existe déjà dans le texte de la page.
...
>[!NOTE]
>
>Le fragment de code ci-dessus n’est utilisé qu’à titre d’exemple pour la commande. Utilisez la variable **Image** plutôt que le composant `img src` référence utilisée ci-dessus.

Dans AEM, vous pouvez utiliser une combinaison de la variable **Texte de remplacement** et **Description** dans la boîte de dialogue de configuration de l’image, comme dans [Comment procéder - Contenu non textuel (1.1.1)](#how-to-meet-non-text-content).
...
Le W3C propose plusieurs suggestions, telles que les suivantes. Chacune de ces approches a ses mérites et ses inconvénients.

    * Puzzles logiques
    * L’utilisation de la sortie son au lieu des images
    * Comptes d’utilisation limités et filtres de spam.

* Images d’arrière-plan :

Ces images sont réalisées à l’aide de feuilles de style en cascade (CSS) plutôt que par HTML. Il n’est pas possible de spécifier une autre valeur de texte. Par conséquent, les images d’arrière-plan ne doivent pas fournir d’informations textuelles importantes. Si elles le font, ces informations doivent également être fournies dans le texte de la page.
...
* [W3C: HTML5 Techniques permettant de fournir des équivalents textuels utiles](https://html.spec.whatwg.org/multipage/images.html#alt)
...
Ces informations traitent du contenu web qui est *en fonction du temps*. Cela concerne le contenu que l’utilisateur peut lire (contenu vidéo, audio et animé, par exemple) et qui peut être pré-enregistré ou en direct.
...
* Si le contenu est un contenu audio pré-enregistré sans vidéo (podcast par exemple) :
   ...
   * fournissez un lien juste avant ou après le contenu vers une transcription textuelle du contenu audio ;
   ...
   La transcription doit être une page de HTML avec un équivalent textuel de tout le contenu non parlé important et parlé. Il doit également indiquer qui parle, une description du décor, des expressions vocales et une description de tout autre son significatif.
...
L’insertion de contenu multimédia, en particulier de contenu de Flash, dans vos pages web AEM est similaire à l’insertion d’une image. Cependant, comme le contenu multimédia est bien plus qu’une image fixe, il existe différents paramètres et options pour contrôler la lecture du contenu multimédia.
...
Dans la mesure du possible, utilisez le sous-titrage codé. Il permet aux utilisateurs d’afficher ou non les sous-titres.

Pour les sous-titres non intégrés, créez et fournissez un fichier de sous-titres synchronisés dans un format approprié, tel que [SMIL](https://www.w3.org/AudioVideo/), ainsi que le fichier vidéo.

Voir les tutoriels dans [En savoir plus - Sous-titres (pré-enregistrés) (1.2.2)](#more-information-captions-pre-recorded). Veillez à fournir une note pour informer les utilisateurs que des sous-titres sont disponibles pour la vidéo.
...
Les personnes aveugles ou malvoyantes rencontrent des obstacles à l’accessibilité si les informations contenues dans une vidéo ou une animation ne sont fournies que visuellement. Ou, si la bande sonore ne fournit pas suffisamment d’informations pour permettre de comprendre ce qui se passe visuellement.
...
1. Incluez une audio-description supplémentaire pour le contenu vidéo. Vous pouvez effectuer cette approche de trois façons :
   ...
   * Fournissez une nouvelle piste audio supplémentaire et facultative contenant la piste audio d’origine, mais également des informations audio supplémentaires sur les modifications dans la scène.
      ...
      * Les utilisateurs peuvent basculer entre la piste audio existante (qui *ne fait pas* contiennent une description audio) et la nouvelle piste audio (qui *does* contient une description audio).
      * Cette méthode permet d’éviter toute perturbation des utilisateurs qui n’ont pas besoin d’une description supplémentaire.
...
AEM facilite la construction de pages web à l’aide des éléments de HTML appropriés. Ouvrez le contenu de votre page dans l’éditeur de texte enrichi (un composant Texte) et utilisez la variable **Format** pour spécifier l’élément structurel approprié (par exemple, paragraphe et en-tête).
...
Assurez-vous que vos pages web disposent de la structure appropriée en procédant comme suit :
...
Mettez le texte en évidence à l’aide des éléments &lt;strong> ou &lt;em>. N’utilisez pas de titres au sein des paragraphes.
    ...
    * Cliquez sur l’icône **B** (pour &amp;lt;strong&amp;gt;) ou sur l’icône **I** (pour &amp;lt;em&amp;gt;) affichée dans le panneau **Propriétés** (assurez-vous que le HTML est sélectionné).
...
>[!NOTE]
Le `<dl>` n’est pas pris en charge par l’éditeur de texte enrichi.
...
Les tableaux de données doivent être identifiés à l’aide d’éléments de tableau de HTML :
    ...
    * un `&lt;table>Élément `
    * a `&lt;tr>Élément ` pour chaque ligne du tableau
    * a `&lt;th>Élément ` pour chaque en-tête de ligne et de colonne
    * a `&lt;td>Élément ` pour chaque cellule de données
...
En outre, les tableaux accessibles utilisent les éléments et attributs suivants :
    ...
    * Le `&lt;caption>` sert à fournir une légende visible pour le tableau. Les légendes apparaissent par défaut centrées au-dessus du tableau, mais peuvent être positionnées de manière appropriée à l’aide de CSS. La légende est associée au tableau par programmation, ce qui en fait une méthode utile pour fournir une introduction au contenu.
    * Le `&lt;h3 class=&quot;summary&quot;>L’élément ` aide les utilisateurs non voyants à comprendre plus facilement les informations présentées dans un tableau, en fournissant une synthèse de ce qu’un utilisateur voyant peut voir. Cela s’avère particulièrement utile lorsque des mises en page de tableau complexes ou non conventionnelles sont utilisées (cet attribut n’est pas affiché dans le navigateur, il est uniquement lu pour les technologies d’assistance).
    * L’attribut &quot;scope&quot; du `&lt;th>L’élément ` sert à indiquer si une cellule représente un en-tête pour une ligne particulière ou pour une colonne particulière. Une approche similaire consiste à utiliser les attributs header et id dans des tableaux complexes, où les cellules de données peuvent être associées à un ou plusieurs en-têtes.
...
Lors de l’ajout d’une **Tableau**, vous pouvez configurer **Propriétés du tableau** à l’aide de la boîte de dialogue .
    ...
    * Idéalement, supprimez toutes les valeurs par défaut pour **Width**, **Height**, **Border**, **Marge intérieure des cellules**, **Espacement des cellules**. En effet, ces propriétés peuvent être définies dans une feuille de style globale.
...
Parfois, lorsqu’il existe des tableaux complexes comportant deux niveaux ou plus d’en-têtes, les propriétés de tableau de base peuvent ne pas suffire à fournir toutes les informations structurelles nécessaires. Pour ce type de tableaux complexes, des relations directes doivent être créées entre les en-têtes et leurs cellules associées à l’aide de la fonction **header** et **id** attributs. Par exemple, dans le tableau ci-dessous, les attributs header et id correspondent pour créer une association de programmation pour les utilisateurs de technologies d’assistance.
...
Les concepteurs et conceptrices concentrent généralement leurs efforts sur le côté visuel (couleur, forme, style du texte, ou position absolue ou relative d’un élément du contenu) de la présentation des informations. Il peut s’agir de puissantes techniques de conception pour véhiculer l’information, mais les personnes aveugles ou malvoyantes peuvent ne pas être en mesure d’accéder à l’information qui nécessite l’identification visuelle d’attributs tels que la position, la couleur ou la forme.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>Mercredi 17 mars<sup>th</p> </td>
   <td><p>SIÈGES DISPONIBLES</p> </td>
  </tr>
  <tr>
...
S’il n’est pas possible de respecter les niveaux de contraste recommandés, fournissez un lien vers une version équivalente alternative de la page (qui ne présente aucun problème de contraste des couleurs). Vous pouvez également laisser l’utilisateur régler le contraste du modèle de couleurs de la page selon ses besoins.
...
Certains utilisateurs peuvent trouver le contenu qui bouge gênant et rend difficile de se concentrer sur d’autres parties de la page. En outre, un tel contenu peut s’avérer difficile à lire pour les personnes qui ont du mal à suivre le texte en mouvement.
...
* Fournissez un moyen de suspendre le défilement du contenu afin que les utilisateurs disposent de suffisamment de temps pour le lire. Par exemple, des bannières d’actualités ou du texte mis à jour automatiquement.
...
Lorsqu’une nouvelle page HTML est créée dans AEM, vous pouvez spécifier son titre. Assurez-vous que le titre décrit correctement le contenu de la page, de sorte que les visiteurs puissent rapidement identifier si le contenu correspond à leurs besoins.
...
* Niveau A
* Fonction du lien (selon le contexte) : L’objectif de chaque lien peut être déterminé à partir du texte du lien seul ou du texte du lien associé à son contexte de lien déterminé par programmation. L’exception est lorsque l’objectif du lien est ambigu pour les utilisateurs en général.

#### Objectif – Fonction du lien (selon le contexte) (2.4.4) {#purpose-link-purpose-in-context}

Pour tous les utilisateurs, qu’ils aient une déficience ou non, il est essentiel d’indiquer clairement la direction d’un lien par le biais du texte du lien approprié. Cette conception permet aux utilisateurs de décider s’ils souhaitent réellement suivre un lien. Pour les utilisateurs voyants, un texte de lien significatif est utile lorsqu’il existe plusieurs liens sur une page (en particulier si la page contient beaucoup de texte), car un texte de lien significatif fournit une indication plus claire de la fonctionnalité de la page cible. Bien que les utilisateurs de technologies d’assistance, qui peuvent générer une liste de tous les liens sur une seule page, puissent plus facilement comprendre le texte du lien hors contexte.
...
* Le texte contenu dans l’attribut title n’est disponible que sous la forme d’une info-bulle contextuelle pour les utilisateurs de la souris. Il n’est pas accessible à l’aide du clavier.
...
Vous pouvez également utiliser des scripts afin qu’une quantité minimale de texte soit fournie dans le lien lui-même. Mais lors de l’activation d’un contrôle approprié positionné en haut de la page, le texte du lien est *expand* plus de détails. Une approche similaire consiste à utiliser CSS pour *masquer* le lien complet des utilisateurs voyants, mais il est toujours affiché en plein écran pour les utilisateurs de lecteurs d’écran. Cela ne relève pas du présent document, mais vous trouverez plus d&#39;informations sur la façon d&#39;y parvenir dans la section [En savoir plus - Fonction du lien (selon le contexte) (2.4.4)](#more-information-link-purpose-in-context) .
...
* En revanche, une page à restituer en anglais américain doit respecter la norme suivante :
...
Pour certains composants de formulaire, il est également possible de masquer visuellement les libellés à l’aide de la fonction **Masquer le titre** . Les libellés masqués de cette manière sont toujours disponibles pour les dispositifs d’assistance, mais ne s’affichent pas à l’écran. Bien qu’il s’agisse d’une bonne approche dans certains cas, il est préférable d’inclure un libellé visuel dans la mesure du possible. Certains utilisateurs peuvent consulter une petite section de l’écran (un champ à la fois) et avoir besoin des libellés pour identifier correctement le champ.
...
Cependant, il n’existe aucune association par programmation entre le titre du groupe et les boutons radio eux-mêmes. Les éditeurs de modèles doivent placer le titre dans la balise `fieldset` et `legend` pour créer cette association, vous ne pouvez le faire qu’en modifiant le code source de la page. Un administrateur système peut également ajouter la prise en charge de ces éléments afin qu’ils apparaissent dans la boîte de dialogue **Propriétés du champ** (voir [Ajout de la prise en charge d’éléments et d’attributs HTML supplémentaires](/help/sites-administering/rte-accessible-content.md#add-support-for-more-html-elements-and-attributes)).
...
![Ajout d’informations supplémentaires (le mot requis) pour les utilisateurs de lecteurs d’écran dans le champ &quot;Titre&quot;.](assets/chlimage_1-25a.png)
